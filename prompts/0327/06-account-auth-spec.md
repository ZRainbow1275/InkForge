...
- [x] `/account` 页面正常显示当前账户信息 (头像/名称/邮箱/简介)
- [x] 可创建新的本地账户 (Zod 验证, 存储到 IndexedDB)
- [x] 可在多个本地账户之间切换 (更新本地当前账户指针)
- [x] Settings account Tab 显示账户切换与安全占位卡片
- [x] 头像上传和 Canvas 裁剪 (200x200) 功能正常
- [x] 账户数据导出功能正常 (GDPR 风格 JSON)
- [x] Hub 顶部显示 36x36 头像按钮 (有头像显示图片, 无头像显示首字母)
- [x] 点击 Hub 头像跳转到 `/account` 页面
- [x] 所有数据来自 IndexedDB (无 Mock 数据)
...
- P1-06 已按本规范完成并重新验证：`AccountWelcome.vue`、`stores/account.ts`、`utils/db.ts`、`router/index.ts`、`HubView.vue`、`SettingsView.vue` 构成本地账户垂直切片。
- 账户数据真实落在 IndexedDB `accounts` 表；头像真实走 `File` -> Canvas 200x200 PNG -> `db.assets` -> `accounts.avatarAssetId`，不使用远程头像、mock 数据或 `window.prompt()`。
- 当前应用没有独立 `components/hub/HubHeader.vue`，Hub 头像入口按实际结构实现于 `HubView.vue` topbar，并保留 36x36 本地头像/首字母 fallback 与 `/account` 导航。
- 认证边界保持严格：远程同步、本地密码、Windows Hello 均为 disabled/即将推出，不伪装已上线。
- 验证通过：`P1_06_STATIC_OK`、`pnpm exec vue-tsc --noEmit`、`pnpm exec eslint src --ext .ts,.tsx,.vue --quiet`、`pnpm build`。

...
describe('Zhihu image artifact manifests', () => {
    it('infers Zhihu-supported image formats and host status from final artifact URLs', () => {
        ...
        expect(inferZhihuImageHostStatus('https://169.254.1.2/diagram.png')).toBe('local-only')
        expect(inferZhihuImageHostStatus('https://100.64.0.1/diagram.png')).toBe('local-only')
        expect(inferZhihuImageHostStatus('https://198.18.0.1/diagram.png')).toBe('local-only')
        expect(inferZhihuImageHostStatus('https://[fc00::1]/diagram.png')).toBe('local-only')
        expect(inferZhihuImageHostStatus('https://[fe80::1]/diagram.png')).toBe('local-only')
        expect(inferZhihuImageHostStatus('blob:http://localhost/diagram')).toBe('local-only')
        expect(inferZhihuImageHostStatus('article.assets/diagram.png')).toBe('local-only')
        expect(inferZhihuImageHostStatus('http://cdn.example.com/diagram.png')).toBe('blocked')
        expect(inferZhihuImageHostStatus('//cdn.example.com/diagram.png')).toBe('blocked')

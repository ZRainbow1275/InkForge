...
This directory now also contains `flagship-kiln-paste-safe.html`, generated by the same real
`emit-flagship-artifacts` pipeline.

- Purpose: additive ordinary-paste compatibility candidate after the exact `flagship-kiln.html`
  WeChat ordinary Ctrl+V negative proof.
- Source preset: `flagship-kiln-paste-safe`.
- Style choice: `wechat-flagship-kiln-paste-safe`.
- First SVG module: `cover-title`.
- Preserved Kiln identity: Kiln palette, creative persona, `divider-forge`, and the flagship HTML
  block chain.
- Artifact SHA-256:
  `338f47e5237131b8e51cf8637d0430b91a8a5e7de0d2f8ccf0625880c062b491`.
- Artifact counts: `svgCount=35`, `dataInkSvgCount=3`, `dataInkBlockCount=23`.

Local CloakBrowser contenteditable proof with Windows CF_HTML plus `keybd_event` Ctrl+V preserved
the candidate's rich HTML/SVG. This is local candidate evidence only. It does not prove WeChat

...
- The original rendering-engine baseline is now implemented in the current nested app with real code-block, table, image, context-menu, preview, export, and quality-detection paths.
- KaTeX and Mermaid are no longer only theoretical optional dependencies: `katex@0.16.45` and `mermaid@11.14.0` are installed and loaded through Vite-visible dynamic imports. KaTeX CSS is loaded globally through `main.ts`.
- Preview and platform conversion now share `renderMarkdownWithOptionalEnhancements()`, so math and Mermaid blocks are processed before WeChat, Zhihu, and Xiaohongshu adapters apply their platform-specific styles and sanitization.
- The current implementation intentionally uses the existing `optional-renderers.ts` pipeline instead of adding standalone `Mathematics.ts`, `MermaidBlock.ts`, or `MermaidNodeView.vue` files. Those standalone TipTap node views remain future architecture work under the newer 0420 rendering-core spec.
- Verified on 2026-04-29 with type-check, ESLint, targeted static/SFC/dependency checks, and production build.

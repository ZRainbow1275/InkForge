...
> **版本**：v3.1 (渲染增强版)
> **日期**：2026-02-27
> **来源**：doocs/md 项目深度分析 + lyricat/wechat-format 参考
...
**解决方案**：使用 `transform` 替代

```typescript
html = html.replace(
    /([^-])top:(.*?)em/g,
    '$1transform: translateY($2em)'
)
```
...
| **布局** | `display: block/inline-block/table/table-cell`, `width`, `max-width`, `float` |
...
| `top/left/right/bottom` | 部分失效 | `transform: translate()` |
...
|------|--------------|------|------|--------|
| NOTE | `> [!NOTE]` | ℹ️ | `#0969da` | `#ddf4ff` |
| TIP | `> [!TIP]` | 💡 | `#1a7f37` | `#dafbe1` |
| IMPORTANT | `> [!IMPORTANT]` | ❗ | `#8250df` | `#fbefff` |
| WARNING | `> [!WARNING]` | ⚠️ | `#9a6700` | `#fff8c5` |
| CAUTION | `> [!CAUTION]` | 🔴 | `#d1242f` | `#ffebe9` |
...
- [ ] CSS 只使用简单选择器（元素、类、ID）
- [ ] 所有 CSS 变量有对应的正则替换规则（含 `var(--name, fallback)` 格式）

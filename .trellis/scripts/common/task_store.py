...
def cmd_archive(args: argparse.Namespace) -> int:
    ...
    try:
        source_task_path = task_dir.relative_to(repo_root).as_posix()
    except ValueError:
    ...
    if "archived_to" in result:
        ...
        if not getattr(args, "no_commit", False):
            _auto_commit_archive(dir_name, repo_root, source_task_path)
...
def _auto_commit_archive(
    ...
    paths = safe_archive_paths_to_add(repo_root, archived_source_path)
    if not paths:

...
const EVIDENCE_DIR = path.resolve(process.env.TEMP || '.', 'inkforge-current-wechat-evidence');
...
const SEED_MARKDOWN = process.env.INKFORGE_E2E_SEED_MARKDOWN_FILE
  ? fs.readFileSync(process.env.INKFORGE_E2E_SEED_MARKDOWN_FILE, 'utf8')
  : DEFAULT_SEED_MARKDOWN;
const SEED_COMPONENT_IDS = process.env.INKFORGE_E2E_SEED_MARKDOWN_FILE
  ? JSON.parse(fs.readFileSync(`${process.env.INKFORGE_E2E_SEED_MARKDOWN_FILE}.meta.json`, 'utf8')).componentIds
  : ['StatBlock'];
const COMPONENT_PROOF_MARKERS = {
  MpProfile: '公众号 / 作者名片',
  AuthorBlock: '仓库自有校准文稿',
  QRCodeBlock: '二维码',
  TipBlock: '提示框验收',
  InfoGrid: '信息网格验收',
  TableBlock: '表格验收',
  ...
  StatBlock: '验收覆盖率',
  GalleryBlock: '图集验收',
  CitationBlock: '所有结论仅绑定当前 release',
  SongBlock: '演唱：Frank C. Stanley',
  ImageBlock: '公开项目图形用于图片与题注链路验收',
  LinkBlock: '链接卡片验收',
  ArticleBlock: '关联文章验收',
  ContactCard: '联系人 / 名片',
  WechatMediaBlock: '微信原生媒体仍需在公众号编辑器内确认绑定',
};
const SEED_COMPONENT_PROOF_MARKERS = SEED_COMPONENT_IDS.map(id => COMPONENT_PROOF_MARKERS[id] || id);
...
describe('InkForge — SVG flagship typesetting (PR7, multi-round, real binary)', () => {
  ...
  it.only('completes the visible release editor experiment before export acceptance', async function () {
    this.timeout(300_000);
    expect(exportReady, seedFailureReason || 'release editor experiment precondition').to.equal(true);

    const targetText = '正文连续阅读流用于检查移动端行宽';
    const selectorByFormat = {
      'heading-1': 'h1',
      'heading-2': 'h2',
      'heading-3': 'h3',
      'heading-4': 'h4',
      ...
      'bullet-list': 'ul:not([data-type="taskList"]) li',
      'ordered-list': 'ol li',
      'task-list': 'ul[data-type="taskList"] li',
      'code-block': 'pre code',
    };

    const selectTargetText = async () => {
      const result = await browser.execute((needle) => {
        const root = document.querySelector('.editor-paper .tiptap-content .ProseMirror');
        if (!root) return { ok: false, reason: 'editor-missing' };
        ...
        for (let node = walker.nextNode(); node; node = walker.nextNode()) {
          ...
          return { ok: true, tag: node.parentElement?.tagName || '' };
        }
        return { ok: false, reason: 'text-missing' };
      }, targetText);
      expect(result.ok, `select visible editor text: ${JSON.stringify(result)}`).to.equal(true);
      const toolbar = await browser.$('.editor-paper .floating-toolbar[role="toolbar"][aria-label="文本格式工具栏"]');
      await toolbar.waitForDisplayed({ timeout: 8_000, interval: 100 });
      const semanticSelect = await toolbar.$('select.ft-block-select[aria-label="块级语义"]');
      expect(await semanticSelect.isEnabled(), 'block semantic selector is enabled for prose').to.equal(true);
      return semanticSelect;
    };

    const waitForFormat = async (format) => {
      const selector = selectorByFormat[format];
      await browser.waitUntil(
        async () => browser.execute((needle, blockSelector) => Array.from(
          document.querySelectorAll(`.editor-paper .tiptap-content .ProseMirror ${blockSelector}`),
        ).some(element => (element.textContent || '').includes(needle)), targetText, selector),
        { timeout: 8_000, interval: 100, timeoutMsg: `${format} did not appear in the visible release editor` },
      );
    };
    ...
    const waitForParagraph = async () => {
      await browser.waitUntil(
        async () => browser.execute((needle) => Array.from(
          document.querySelectorAll('.editor-paper .tiptap-content .ProseMirror p'),
        ).some(element => (element.textContent || '').includes(needle) && !element.closest('.ink-component-card')), targetText),
        { timeout: 8_000, interval: 100, timeoutMsg: 'semantic experiment target did not return to a paragraph' },
      );
    };

    const saveAndReadEditorState = async () => {
      const editor = await browser.$('.editor-paper .tiptap-content .ProseMirror');
      ...
      await browser.waitUntil(
        async () => {
          state = await browser.execute(() => {
            ...
            return {
              body: editorStore?.currentContent?.body || '',
              articleId: editorStore?.currentContent?.articleId || '',
              status: editorStore?.status || '',
              saveState: document.querySelector('.status-pill')?.getAttribute('data-save-state') || '',
            };
          });
          return state.articleId === seededArticleId && state.saveState === 'clean' && state.status === 'ready';
        },
        { timeout: 15_000, interval: 150, timeoutMsg: `release editor did not persist cleanly: ${JSON.stringify(state)}` },
      );
      return state;
    };
    ...
    for (const format of [
      ...
      'blockquote', 'bullet-list', 'ordered-list', 'task-list', 'code-block',
    ]) {
      const semanticSelect = await selectTargetText();
      await semanticSelect.selectByAttribute('value', format);
      await waitForFormat(format);
      if (format === 'heading-3') {
        await browser.saveScreenshot(path.join(EVIDENCE_DIR, 'editor-semantic-toolbar-20260802.png'));
      }
      const editor = await browser.$('.editor-paper .tiptap-content .ProseMirror');
      await browser.execute((surface) => surface.focus(), editor);
    ...
    const h6Select = await selectTargetText();
    await h6Select.selectByAttribute('value', 'heading-6');
    await waitForFormat('heading-6');
    const headingState = await saveAndReadEditorState();
    expect(headingState.body, 'H6 semantic is saved as Markdown authority').to.include(`###### ${targetText}`);

    await browser.refresh();
    await waitForMainWindow();
    const reopenedHeading = await reachWorkstationExport(seededArticleId);
    expect(reopenedHeading.ready, reopenedHeading.reason || 'saved H6 draft reopens').to.equal(true);
    await waitForFormat('heading-6');

    const paragraphSelect = await selectTargetText();
    await paragraphSelect.selectByAttribute('value', 'paragraph');
    await waitForParagraph();
    const restoredState = await saveAndReadEditorState();
    expect(restoredState.body, 'restored paragraph remains in Markdown authority').to.include(targetText);
    expect(restoredState.body, 'restored paragraph no longer carries H6 syntax').not.to.include(`###### ${targetText}`);

    await browser.refresh();
    await waitForMainWindow();
    const reopenedParagraph = await reachWorkstationExport(seededArticleId);
    expect(reopenedParagraph.ready, reopenedParagraph.reason || 'restored paragraph draft reopens').to.equal(true);
    await waitForParagraph();

    const deliveryTrigger = await browser.$('button[aria-label="配置文前歌曲、阅读信息与文末名片"]');
    await deliveryTrigger.scrollIntoView({ block: 'center', inline: 'nearest' });
    await deliveryTrigger.waitForClickable({ timeout: 8_000 });
    await deliveryTrigger.click();
    const deliveryDialog = await browser.$('.delivery-settings-overlay .delivery-settings-dialog[role="dialog"][aria-labelledby="delivery-settings-title"]');
    await deliveryDialog.waitForDisplayed({ timeout: 8_000 });
    expect(await (await browser.$('.export-panel')).isExisting(), 'delivery configuration does not open ExportModal').to.equal(false);
    await browser.saveScreenshot(path.join(EVIDENCE_DIR, 'editor-delivery-settings-20260802.png'));
    const closeDelivery = await browser.$('button[aria-label="关闭文章组件与交付配置"]');
    if (await closeDelivery.isExisting()) await closeDelivery.click();
    await (await browser.$('.delivery-settings-dialog')).waitForExist({ reverse: true, timeout: 8_000 });
    ...
    const openLibrary = await browser.$('button[aria-label="打开组件库"]');
    await openLibrary.scrollIntoView({ block: 'center', inline: 'nearest' });
    await openLibrary.waitForClickable({ timeout: 8_000 });
    await openLibrary.click();
    let library = await browser.$('.component-library[role="dialog"][aria-labelledby="component-library-title"]');
    await library.waitForDisplayed({ timeout: 8_000 });
    expect(await (await browser.$('.export-panel')).isExisting(), 'component entry opens the writing library, not ExportModal').to.equal(false);
    await browser.saveScreenshot(path.join(EVIDENCE_DIR, 'editor-component-library-20260802.png'));

    const search = await library.$('input[aria-label="搜索组件"]');
    await search.setValue('提示框');
    const tipOption = await library.$('aside[role="listbox"][aria-label="组件列表"] button[role="option"]');
    expect(await tipOption.getText(), 'real TipBlock option is visible in the writing library').to.include('提示框');
    await tipOption.click();
    await (await library.$('.component-fields input[type="text"]')).setValue('EDITOR-EXPERIMENT-TIP');
    await (await library.$('.component-fields textarea')).setValue('release 编辑器真实插入');
    const insertButton = await library.$('.primary-action');
    await insertButton.waitForEnabled({ timeout: 8_000 });
    await insertButton.click();
    await library.waitForExist({ reverse: true, timeout: 8_000 });

    const insertedSelector = '.ink-component-card[data-ink-component-id="TipBlock"][data-ink-component-source*="EDITOR-EXPERIMENT-TIP"]';
    let insertedCard = await browser.$(insertedSelector);
    await insertedCard.waitForDisplayed({ timeout: 10_000 });
    expect(await insertedCard.getAttribute('data-ink-component-status'), 'inserted component is ready').to.equal('ready');
    await (await insertedCard.$('.ink-component-card__edit')).click();
    library = await browser.$('.component-library[role="dialog"][aria-labelledby="component-library-title"]');
    await library.waitForDisplayed({ timeout: 8_000 });
    await (await library.$('.component-fields textarea')).setValue('EDITOR-EXPERIMENT-TIP-UPDATED');
    const updateButton = await library.$('.primary-action');
    expect(await updateButton.getText(), 'existing editor card uses the update path').to.include('更新组件');
    await updateButton.click();
    await library.waitForExist({ reverse: true, timeout: 8_000 });
    insertedCard = await browser.$(`${insertedSelector}[data-ink-component-source*="EDITOR-EXPERIMENT-TIP-UPDATED"]`);
    await insertedCard.waitForDisplayed({ timeout: 10_000 });

    await insertedCard.click();
    await browser.waitUntil(
      async () => (await insertedCard.getAttribute('class')).includes('is-selected'),
      { timeout: 5_000, interval: 100, timeoutMsg: 'inserted component did not become an atomic selection' },
    );
    await browser.keys('Delete');
    await insertedCard.waitForExist({ reverse: true, timeout: 8_000 });
    const finalState = await saveAndReadEditorState();
    fs.writeFileSync(path.join(EVIDENCE_DIR, 'editor-roundtrip-body-20260802.md'), finalState.body, 'utf8');
    expect(finalState.body, 'editor experiment component is removed through the real atomic node path')
      .not.to.include('EDITOR-EXPERIMENT-TIP');
    for (const semantic of [
      ...
      '~~删除线~~',
      '`inlineCode()`',
      '[公开仓库链接](https://github.com/ZRainbow1275/InkForge)',
      '<u>下划线</u>',
      '==高亮==',
      ...
      expect(finalState.body, `persisted Markdown authority keeps ${semantic}`).to.include(semantic);
    }
    expect(finalState.body, 'footnote definition survives the release editor round trip')
      .to.match(/\[\^inkforge\]: .*https:\/\/github\.com\/ZRainbow1275\/InkForge/);
    expect(finalState.body, 'Tiptap round trip must not escape inline Markdown authority')
      .not.to.match(/\\\*\\\*|\\`inlineCode|\\\[公开仓库链接/);
    expect(finalState.body, 'footnotes must not be rewritten as wikilinks').not.to.include('#wikilink-');
    expect(finalState.body, 'normalized footnote section chrome is not persisted').not.to.include('## Footnotes');
    for (const componentId of SEED_COMPONENT_IDS) {
      expect(finalState.body, `registered ${componentId} source survives the release editor round trip`)
        .to.include(`<${componentId} `);
    }
    expect(
      await browser.$$('.editor-paper .ink-component-card[data-ink-component-id="TipBlock"]'),
      'the original corpus TipBlock remains after experiment cleanup',
    ).to.have.length(1);

    await openFlagshipExportPreview('赤陶旗舰');
    const artifactProbe = await browser.execute((proofMarkers) => {
      const root = document.querySelector('.export-panel .preview-render');
      const text = root?.textContent || '';
      const highlight = Array.from(root?.querySelectorAll('mark') || [])
        .find(node => (node.textContent || '').trim() === '高亮');
      return {
        strong: root?.querySelectorAll('strong, b').length || 0,
        emphasis: root?.querySelectorAll('em, i').length || 0,
        strike: root?.querySelectorAll('del, s, strike').length || 0,
        inlineCode: Array.from(root?.querySelectorAll('code') || [])
          .some(node => (node.textContent || '').includes('inlineCode()')),
        linkLabel: text.includes('公开仓库链接'),
        linkFootnoteHeading: Array.from(root?.querySelectorAll('h4') || [])
          .some(node => (node.textContent || '').trim() === '引用链接'),
        linkFootnoteUrl: text.includes('https://github.com/ZRainbow1275/InkForge'),
        linkFootnoteRef: Array.from(root?.querySelectorAll('sup') || [])
          .some(node => /^\[\d+\]$/.test((node.textContent || '').trim())),
        rawExternalLink: Boolean(root?.querySelector('a[href="https://github.com/ZRainbow1275/InkForge"]')),
        underline: root?.querySelectorAll('u').length || 0,
        highlight: Boolean(highlight),
        highlightBackground: highlight ? getComputedStyle(highlight).backgroundColor : '',
        subscript: root?.querySelectorAll('sub').length || 0,
        superscript: root?.querySelectorAll('sup').length || 0,
        table: root?.querySelectorAll('table').length || 0,
        codeBlock: root?.querySelectorAll('pre').length || 0,
        image: root?.querySelectorAll('img').length || 0,
        svg: root?.querySelectorAll('[data-ink-svg] svg').length || 0,
        missingComponentProofMarkers: proofMarkers.filter(marker => !text.includes(marker)),
        literalMarkdown: /\\\*\\\*加粗|\*\*加粗\*\*|\[公开仓库链接\]\(/.test(text),
        hasFinalSentinel: text.includes('文末校准哨兵：真实渲染、真实跑通、零模拟。'),
      };
    }, SEED_COMPONENT_PROOF_MARKERS);
    expect(artifactProbe.strong, 'release WeChat artifact renders bold semantically').to.be.greaterThan(0);
    expect(artifactProbe.emphasis, 'release WeChat artifact renders italic semantically').to.be.greaterThan(0);
    expect(artifactProbe.strike, 'release WeChat artifact renders strike semantically').to.be.greaterThan(0);
    expect(artifactProbe.inlineCode, 'release WeChat artifact renders inline code semantically').to.equal(true);
    expect(artifactProbe.linkLabel, 'release WeChat artifact keeps the authoritative link label').to.equal(true);
    expect(artifactProbe.linkFootnoteHeading, 'release WeChat artifact appends the citation heading').to.equal(true);
    expect(artifactProbe.linkFootnoteUrl, 'release WeChat artifact keeps the authoritative URL in citations').to.equal(true);
    expect(artifactProbe.linkFootnoteRef, 'release WeChat artifact adds a visible citation reference').to.equal(true);
    expect(artifactProbe.rawExternalLink, 'release WeChat artifact normalizes the external anchor to a citation').to.equal(false);
    expect(artifactProbe.underline, 'release WeChat artifact renders underline semantically').to.be.greaterThan(0);
    expect(artifactProbe.highlight, 'release WeChat artifact renders highlight semantically').to.equal(true);
    expect(artifactProbe.highlightBackground, 'release WeChat artifact keeps a visible highlight background')
      .not.to.be.oneOf(['', 'transparent', 'rgba(0, 0, 0, 0)']);
    expect(artifactProbe.subscript, 'release WeChat artifact renders subscript semantically').to.be.greaterThan(0);
    expect(artifactProbe.superscript, 'release WeChat artifact renders superscript/footnotes semantically').to.be.greaterThan(0);
    expect(artifactProbe.table, 'release WeChat artifact keeps the table').to.be.greaterThan(0);
    expect(artifactProbe.codeBlock, 'release WeChat artifact keeps the code block').to.be.greaterThan(0);
    expect(artifactProbe.image, 'release WeChat artifact keeps the real image').to.be.greaterThan(0);
    expect(artifactProbe.svg, 'release WeChat artifact injects the selected preset SVG').to.be.greaterThan(0);
    expect(
      artifactProbe.missingComponentProofMarkers,
      'release WeChat artifact renders every registered writing component by visible proof',
    ).to.deep.equal([]);
    expect(artifactProbe.literalMarkdown, 'release WeChat artifact contains no escaped/literal inline Markdown').to.equal(false);
    expect(artifactProbe.hasFinalSentinel, 'release WeChat artifact reaches the final corpus sentinel').to.equal(true);
    const artifactHtml = await browser.execute(() =>
      document.querySelector('.export-panel .preview-render')?.innerHTML || '');
    expect(artifactHtml, 'release WeChat artifact HTML is available for independent inspection').not.to.equal('');
    fs.writeFileSync(
      path.join(EVIDENCE_DIR, 'editor-roundtrip-artifact-20260802.html'),
      artifactHtml.replace(/[ \t]+$/gm, ''),
      'utf8',
    );
    await browser.saveScreenshot(path.join(EVIDENCE_DIR, 'editor-roundtrip-artifact-20260802.png'));
    await closeExportModal();
  });
  ...
  it('keeps editor components, delivery slots, and all 16 preset geometries aligned with the real WeChat artifact', async function () {
      ...
      components: [
        {
          ...
          url: 'https://commons.wikimedia.org/wiki/File:Auld_Lang_Syne.ogg',
        },
        {
          id: 'native-profile',
          type: 'contact-card',
          ...
          accountId: 'ZRainbow1275/InkForge',
          description: '公开仓库与当前 release 校准说明。',
          profileUrl: 'https://github.com/ZRainbow1275/InkForge',
        },
      ],
    };
    ...
    try {
      ...
      try {
        await browser.waitUntil(
          async () => browser.execute((expectedComponentIds) => {
            ...
            const cards = Array.from(document.querySelectorAll('.tiptap-content .ink-component-card'));
            const componentIds = cards.map(card => card.getAttribute('data-ink-component-id'));
            const text = `${front?.textContent || ''} ${end?.textContent || ''}`;
            return Boolean(front && end) && cards.length === expectedComponentIds.length &&
              expectedComponentIds.every(id => componentIds.includes(id)) &&
              cards.every(card => card.getAttribute('data-ink-component-status') === 'ready') &&
              cards.every(card => card.querySelector('.ink-component-card__visual')) &&
              text.includes('Auld Lang Syne') &&
              text.includes('公开仓库与当前 release 校准说明') && /阅读约\s*\d+\s*分钟/.test(text) &&
              /全文\s*\d+\s*字/.test(text);
          }, SEED_COMPONENT_IDS),
          {
            timeout: 20_000,
            interval: 200,
            timeoutMsg: 'editor delivery projections and all registry component visuals did not converge',
          },
        );
      } catch (error) {
        const diagnostic = await browser.execute(() => ({
          ...
          componentIds: Array.from(document.querySelectorAll('.tiptap-content .ink-component-card'))
            .map(card => card.getAttribute('data-ink-component-id')),
      ...
      const editorProbe = await browser.execute(() => {
        const front = document.querySelector('[data-editor-projection="wechat-front"]');
        const end = document.querySelector('[data-editor-projection="wechat-end"]');
        const componentCards = Array.from(document.querySelectorAll('.tiptap-content .ink-component-card'));
        return {
          ...
          componentIds: componentCards.map(card => card.getAttribute('data-ink-component-id')),
          componentStatuses: componentCards.map(card => card.getAttribute('data-ink-component-status')),
          componentVisualCount: componentCards.filter(card => card.querySelector('.ink-component-card__visual')).length,
          visualLinkCount: componentCards.reduce((count, card) => (
            count + card.querySelectorAll('.ink-component-card__visual a[tabindex="-1"][aria-disabled="true"]').length
          ), 0),
        };
      });
      expect(editorProbe.frontText).to.include('文章值得您享受');
      expect(editorProbe.frontText).to.include('Auld Lang Syne');
      expect(editorProbe.frontText).to.match(/阅读约\s*\d+\s*分钟/);
      expect(editorProbe.frontText).to.match(/全文\s*[1-9]\d*\s*字/);
      expect(editorProbe.endText).to.include('公开仓库与当前 release 校准说明');
      ...
      expect(editorProbe.componentIds).to.deep.equal(SEED_COMPONENT_IDS);
      expect(editorProbe.componentStatuses).to.deep.equal(SEED_COMPONENT_IDS.map(() => 'ready'));
      expect(editorProbe.componentVisualCount).to.equal(SEED_COMPONENT_IDS.length);
      ...
      try {
        await browser.waitUntil(
          async () => browser.execute((proofMarkers) => {
            ...
            return Boolean(
              ...
              component &&
              proofMarkers.every(marker => text.includes(marker)) &&
          ...
          }, SEED_COMPONENT_PROOF_MARKERS),
          {
            timeout: 20_000,
            interval: 250,
            timeoutMsg: 'WeChat artifact did not render song, profile, license, and all registry components together',
          },
        );
      } catch (error) {
        const diagnostic = await browser.execute((proofMarkers) => {
          const preview = document.querySelector('.export-panel .preview-render');
          const text = preview?.textContent || '';
          return {
            ...
            missingProofMarkers: proofMarkers.filter(marker => !text.includes(marker)),
      ...
      const artifactProbe = await browser.execute((proofMarkers) => {
        ...
        const component = Array.from(preview?.querySelectorAll('section > strong') ?? [])
          .find(strong => strong.textContent?.trim() === '100%' &&
            ...
            strong.parentElement?.textContent?.includes('来源：InkForge 本轮验收'))
          ?.parentElement;
        const componentIds = Array.from(preview?.querySelectorAll('[data-ink-writing-component]') ?? [])
          .map(element => element.getAttribute('data-ink-writing-component'));
        const text = preview?.textContent || '';
        return {
      ...
      expect(artifactProbe.songText).to.include('Auld Lang Syne');
      expect(artifactProbe.metricsText).to.match(/阅读约\s*\d+\s*分钟/);
      expect(artifactProbe.metricsText).to.match(/全文\s*[1-9]\d*\s*字/);
      expect(artifactProbe.profileText).to.include('公开仓库与当前 release 校准说明');
      expect(artifactProbe.licenseText).to.include('CC BY 4.0');
      expect(artifactProbe.componentText).to.include('100%');
      expect(artifactProbe.missingProofMarkers, 'all 17 registry components retain visible proof after WeChat sanitization')
        .to.deep.equal([]);
      ...
      const fingerprints = [];
      let previousHtml = '';
      for (const [index, card] of [[clipboardPresetNumber - 1, cards[clipboardPresetNumber - 1]]]) {
      ...
      expect(fingerprints.length, 'requested WeChat preset produced rendered geometry').to.equal(1);
      ...
      fs.writeFileSync(
        path.join(EVIDENCE_DIR, `preset-artifact-${String(clipboardPresetNumber).padStart(2, '0')}-20260802.html`),
        clipboardArtifactHtml,
        'utf8',
      );

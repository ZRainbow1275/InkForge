...
class SecurityPolicyManager {
    ...
    private logAudit(
        ...
        // 同时输出到 logger（展开为 Record<string, unknown> 以满足 logger 签名）
        const logContext: Record<string, unknown> = { ...event }
        if (outcome === 'blocked' || outcome === 'failure') {
            logger.warn('[SecurityAudit]', logContext)
        } else {
            logger.debug('[SecurityAudit]', logContext)
        }
    }

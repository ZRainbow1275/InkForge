...
const WechatDraftLiveRoundTripReceiptSchema = z.object({
  hash: z.union([z.literal(''), z.string().regex(/^[a-f0-9]{32}$/i)]),
  count: z.object({
  ...
  error: z.string().min(1).max(160).nullable(),
  cleanupState: z.enum([
    'not-started',
    'blocked',
    'cleanup-pending',
...
export type WechatDraftLiveRoundTripReceipt = z.infer<typeof WechatDraftLiveRoundTripReceiptSchema>
...
export interface WechatDraftLiveRoundTripInput {
  coverHandle: string
...
export async function runWechatDraftLiveRoundTrip(
  input: WechatDraftLiveRoundTripInput,
): Promise<WechatDraftLiveRoundTripReceipt> {
  ensureTauriRuntime()
  const raw = await tauriInvoke<unknown>('wechat_draft_live_round_trip', {
    input: {
      coverHandle: input.coverHandle.trim(),
      manualCleanupConfirmed: input.manualCleanupConfirmed === true,
    },
  })
  return parseWechatResponse(
    WechatDraftLiveRoundTripReceiptSchema,
    raw,
    'wechat_draft_live_round_trip',
  )
}
...
export async function publishWechatDraft(input: WechatDraftPublishInput): Promise<WechatDraftPublishResult> {
  ...
  let coverHandle = input.coverHandle?.trim()

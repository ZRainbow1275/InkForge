...
- 2026-06-09 `flagship-amber` 已有 CloakBrowser 程序化 `ClipboardEvent('paste')` + `DataTransfer` 的真实微信 PC 编辑器读回：exact artifact 保留 `data-ink-svg=3` / `svg=35`。该证据只覆盖这个 PC channel，不推翻普通 Ctrl+V 阻断，也不能替代手机预览、Dark Mode、封面缩略图、同步、定时发送或发布证明。

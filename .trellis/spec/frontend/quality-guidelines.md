...
- A non-empty `orderedTabs` collection must mount the production `WorkstationTabBar` in `WorkstationView`; an implemented but unmounted component is a blocking route defect. Focus mode may hide it without deleting session state.
- Do not nest pin/close buttons inside a `role="tab"` button. Use separate native sibling buttons in a non-interactive draggable wrapper so Enter/Space remains browser-native; the tab button may own ArrowLeft/ArrowRight/Home/End/Delete/Backspace and must move focus after keyboard close.
- Native Tauri smoke must retain any pre-existing real tab, create two additional drafts through visible Hub controls, verify relative pin/order/route/session/IndexedDB state, close and restore through supported shortcuts, reload, and close only the run-created tabs through production actions.
- Route failure proof must install a real Vue Router guard and verify select, close, and restore rejection leaves the route, selected article, editor body, tab session, and recently-closed queue unchanged. Direct store mutation or synthetic history replacement is not sufficient.
- When content rows are encrypted, inspect only envelope shape/count/id metadata at the raw IndexedDB boundary. Prove readable persistence by reloading and reading the markers through the production article store and visible editor; never expect ciphertext to contain plaintext or add a test-only decrypt path.
- A stateful Workstation run must wait for the visible save indicator to be `clean` before editing or reading a selected document. Store `ready` alone is insufficient while EditorPanel hydration is still pending.
- Every child surface that can select an article, including FileManager and TagBrowser, must call the Workstation-owned async selection transaction. Calling `articleStore.selectArticle()` directly bypasses flush, route rejection, tab activation, and source/target isolation.
- Editor saves must capture content id, article id, title, transcript, Markdown, and a monotonic local revision before scheduling. A same-document persistence echo may update durable state, but it must not hydrate over a newer unsaved editor revision.
- Concurrency acceptance must hold a real IndexedDB `readwrite` transaction while the first production save waits, then type a newer revision through native keyboard input. Do not patch the repository, inject a fake delay, or use a mock storage adapter to manufacture save ordering.
- Native route helpers must wait for Vue transition enter/leave classes to clear and the current `.app-route-shell` opacity to reach its settled state before interacting. Element visibility alone can return a leaving or transparent Settings view and cause false `element not interactable` failures.
- Workstation shortcuts prevent browser defaults only when a real action is available. A single tab must not consume `Ctrl+Tab`, a pinned tab must not consume `Ctrl+W`, an empty restore queue must not consume `Ctrl+Shift+T`, and an absent number target must not consume `Ctrl+1..9`.
- Collapsed panel recovery remains keyboard reachable through a named native button. In strict native encryption mode, sensitive string fields, including an empty body, must persist as the current encrypted envelope; a plaintext or legacy-v0 fallback is not acceptable proof.
- Dark active-tab styling must target the actual native `.workstation-tabbar__tab` descendant, not only its non-interactive wrapper. Keep a focused source/style contract test so future wrapper/button accessibility changes cannot silently drop the visible active state.
- Shared Tauri desktop E2E must select the real main WebView and fail closed until the native host is restored, foreground-focused, and exposes a desktop-interactable viewport before any route proof starts. Never call WebDriver maximize/resize on the Tauri host: host/client sizing can desynchronize and expose a black native surround while the WebView keeps stale bounds. Host/client width and height gaps are symmetric and may differ by at most 8 px for DPI/native-border variance; 9 px on either side must fail a deterministic boundary regression. A real minimize/focus-loss case must restore the same native process/window through the OS restore path, reselect the original WebDriver handle, and verify document focus plus host/client bounds before later tests; do not accept cascaded `element not interactable` failures as unrelated product defects.
...
- Real Tauri persisted-layout rejection smoke must hold a source document in a genuine saving state, trigger the production Profile/layout watcher against a real saved layout row, reject the target with Vue Router, and prove the source was flushed while all route/article/editor/tab/session/layout fields remain unchanged.

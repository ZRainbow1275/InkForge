...
## 2026-06-29 Xiumi Text Operation Section Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed the text operation
  surface exposes standalone `op-text-sec` class markers in addition to paired text toolbar
  controls.
- Extended the existing static detector label `Xiumi text toolbar control residue` so standalone
  `op-text-sec` class/id markers are blocked with the same precise text-toolbar diagnosis.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "text operation sections" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "text operation sections|text toolbar controls|text editing flyout|font and format controls|color selector controls|color selector class controls" --reporter=default`.
- Full validation:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --testTimeout=90000`
  passed with 1 file and 336 tests;
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --testTimeout=90000`
  passed with 36 files and 1313 tests;
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`
  passed;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`
  passed;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  passed with 4653 modules transformed in 34.15s.
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, typography fidelity, Dark Mode, cover thumbnail acceptance,
...
- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed component-depot
  external edit/link controls `dc-ce-out-cell-edit` and `dc-ce-play-cp-link`.
- Added the static detector label `Xiumi component depot external edit link residue` and tightened
  the broad fallback so the reduced external edit/link fixture is not double-reported as generic
  operator-depot residue.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  ...
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot external edit and link controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot external edit and link controls|component depot SVG animation controls|component depot native controls|component depot form input controls|component depot table auxiliary controls|component depot image transform controls|component depot box style controls|component depot typography controls|operator depot item controls" --reporter=default`.
- Verification:
  full platform-rendering regression passed with 1 file and 330 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1307 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  ...
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules and Vite built in 48.13s.
- Verification:
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, link fidelity, external component behavior, Dark Mode, credentialed sync,
  scheduled send, platform preview, public article rendering, public-host acceptance,
  XHS/Zhihu account upload, or publish success.

---

## 2026-06-29 Xiumi Component Depot SVG Animation Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed component-depot
  SVG/animation controls `dc-ce-animation`, `dc-ce-svg-animate`, and
  `dc-ce-svg-animation`.
- Added the static detector label `Xiumi component depot SVG animation residue`, tightened the
  native `dc-ce-svg` detector so it no longer overmatches `dc-ce-svg-*`, and tightened the broad
  fallback so the reduced SVG animation fixture is not double-reported as generic operator-depot
  residue.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because the reduced fixture emitted
  `Xiumi component depot native control residue` and `Xiumi operator depot item residue`:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot SVG animation controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot SVG animation controls|component depot native controls|component depot form input controls|component depot table auxiliary controls|component depot image transform controls|component depot box style controls|component depot typography controls|operator depot item controls" --reporter=default`.
- Verification:
  full platform-rendering regression passed with 1 file and 329 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1306 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  lint, type check, and production build passed:
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules and Vite built in 42.27s.
- Verification:
  release preflight intentionally remains blocked by external gates:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, blocker kinds
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, SVG animation fidelity, mobile SMIL/click interaction, H5 behavior,
  Dark Mode, credentialed sync, scheduled send, platform preview, public article rendering,
  public-host acceptance, XHS/Zhihu account upload, or publish success.

---

## 2026-06-29 Xiumi Component Depot Table Auxiliary Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed component-depot
  table auxiliary controls `dc-ce-table-column-width`, `dc-ce-table-grid`, and
  `dc-ce-table-style-brush`.
- Added the static detector label `Xiumi component depot table auxiliary residue` before image
  transform and broad `Xiumi operator depot item residue` rules, and tightened the broad fallback
  so the reduced table auxiliary fixture is not double-reported as generic operator-depot
  residue.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because the reduced fixture emitted only
  `Xiumi operator depot item residue`:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot table auxiliary controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot table auxiliary controls|component depot table controls|component depot image transform controls|component depot box style controls|component depot typography controls|component depot mobile viewport controls|component depot layout geometry controls|component depot native controls|component depot form input controls|operator depot item controls" --reporter=default`.
- Verification:
  full platform-rendering regression passed with 1 file and 328 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1305 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  lint, type check, and production build passed:
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules and Vite built in 43.44s.
- Verification:
  release preflight intentionally remains blocked by external gates:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, blocker kinds
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, table rendering, column-width fidelity, grid fidelity, style-brush
  fidelity, Dark Mode, credentialed sync, scheduled send, platform preview, public article
  rendering, public-host acceptance, XHS/Zhihu account upload, or publish success.

---

## 2026-06-29 Xiumi Component Depot Box Style Addendum
...
- Added the static detector label `Xiumi component depot box style residue` before the typography
  and broad `Xiumi operator depot item residue` rules, and tightened the broad fallback so the
  reduced box style fixture is not double-reported as generic operator-depot residue.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because the reduced fixture emitted only
  `Xiumi operator depot item residue`:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot box style controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
...
- Verification:
  full platform-rendering regression passed with 1 file and 327 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1304 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  lint, type check, and production build passed:
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules and Vite built in 56.23s.
- Verification:
  release preflight intentionally remains blocked by external gates:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, blocker kinds
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, visual-style fidelity, background rendering, border/shadow fidelity,
  transparency rendering, Dark Mode, credentialed sync, scheduled send, platform preview, public
  article rendering, public-host acceptance, XHS/Zhihu account upload, or publish success.

---

## 2026-06-29 Xiumi Component Depot Typography Control Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed component-depot
  typography controls `dc-ce-font-size-scale`, `dc-ce-paragraph-margin`, `dc-ce-text-all`,
  `dc-ce-text-code`, `dc-ce-text-decoration`, `dc-ce-text-shadow`, and
  `dc-ce-text-shadow-style`.
- Added the static detector label `Xiumi component depot typography control residue` before the
  broad `Xiumi operator depot item residue` fallback, and tightened the broad fallback so the
  reduced typography fixture is not double-reported as generic operator-depot residue.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because the reduced fixture emitted only
  `Xiumi operator depot item residue`:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot typography controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot typography controls|component depot image transform controls|component depot table controls|component depot mobile viewport controls|component depot layout geometry controls|component depot native controls|component depot form input controls|operator depot item controls" --reporter=default`.
- Verification:
  full platform-rendering regression passed with 1 file and 326 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1303 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  lint, type check, and production build passed:
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules and Vite built in 44.39s.
- Verification:
  release preflight intentionally remains blocked by external gates:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, blocker kinds
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, paragraph spacing fidelity, code typography fidelity, text-shadow
  rendering, Dark Mode, credentialed sync, scheduled send, platform preview, public article
  rendering, public-host acceptance, XHS/Zhihu account upload, or publish success.
...
- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed component-depot table
  controls `dc-ce-classic-table-column-width`, `dc-ce-classic-table-grid`,
  `dc-ce-classic-table-merge`, `dc-ce-classic-table-quickly`,
  `dc-ce-classic-table-style`, and `dc-ce-classic-table-width`.
- Added the static detector label `Xiumi component depot table control residue` before the broad
  `Xiumi operator depot item residue` fallback, and tightened the broad fallback so the reduced
  table control component fixture is not double-reported as generic operator-depot residue.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because the reduced fixture emitted only
  `Xiumi operator depot item residue`:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot table controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot table controls|component depot mobile viewport controls|component depot layout geometry controls|component depot native controls|component depot form input controls|operator depot item controls" --reporter=default`.
- Verification:
  full platform-rendering regression passed with 1 file and 324 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1301 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  lint, type check, and production build passed:
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules and Vite built in 1m 6s.
- Verification:
  release preflight intentionally remains blocked by external gates:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, blocker kinds
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, complex table rendering, table merge/width fidelity, mobile viewport
  acceptance, mobile H5 interaction, mobile SMIL/click interaction, Dark Mode, cover thumbnail
  acceptance, credentialed sync, scheduled send, platform preview, public article rendering,
  public-host acceptance, XHS/Zhihu account upload, or publish success.

---

## 2026-06-29 Xiumi Component Depot Mobile Viewport Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed component-depot
  mobile viewport controls `dc-ce-mobile-background`, `dc-ce-mobile-group`,
  `dc-ce-mobile-image`, `dc-ce-mobile-text`, and `dc-ce-mobile-unsupport`.
- Added the static detector label `Xiumi component depot mobile viewport residue` before the broad
  `Xiumi operator depot item residue` fallback, and tightened the broad fallback so the reduced
  mobile viewport component fixture is not double-reported as generic operator-depot residue.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because the reduced fixture emitted only
  `Xiumi operator depot item residue`:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot mobile viewport controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot mobile viewport controls|component depot layout geometry controls|component depot native controls|component depot form input controls|operator depot item controls" --reporter=default`.
- Verification:
  full platform-rendering regression passed with 1 file and 323 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1300 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  lint, type check, and production build passed:
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules and Vite built in 47.78s.
- Verification:
  release preflight intentionally remains blocked by external gates:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, blocker kinds
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, mobile viewport acceptance, mobile H5 interaction, mobile SMIL/click
  interaction, Dark Mode, cover thumbnail acceptance, credentialed sync, scheduled send, platform
  preview, public article rendering, public-host acceptance, XHS/Zhihu account upload, or publish
  success.
...
## 2026-06-29 Xiumi Component Depot Form Input Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed component-depot
  form/input entries such as `dc-ce-input-checkbox`, `dc-ce-input-radio`,
  `dc-ce-input-select`, `dc-ce-input-text`, and `dc-ce-input-multi-line-text`.
- Added the static detector label `Xiumi component depot form input residue` before the broad
  `Xiumi operator depot item residue` fallback, and tightened the broad fallback so the reduced
  form/input component fixture is not double-reported as generic operator-depot residue.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because the reduced fixture emitted only
  `Xiumi operator depot item residue`:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot form input controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot form input controls|component depot native controls|operator depot item controls|attribute board controls" --reporter=default`.
- Verification:
  full platform-rendering regression passed with 1 file and 321 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1298 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  lint, type check, and production build passed:
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules and Vite built in 39.66s.
- Verification:
  release preflight intentionally remains blocked by external gates:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, blocker kinds
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, mobile form interaction, mobile interaction, Dark Mode, cover thumbnail
  acceptance, credentialed sync, H5/form interaction, native component acceptance, scheduled send,
  platform preview, public article rendering, public-host acceptance, XHS/Zhihu account upload, or
  publish success.

## 2026-06-29 Xiumi Component Depot Layout Geometry Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed component-depot
  layout/geometry entries such as `dc-ce-layout-free`, `dc-ce-layout-fixed-aspect-ratio`,
  `dc-ce-layout-scroll-direction`, `dc-ce-layout-column`, `dc-ce-layout-hidden`,
  `dc-ce-layout-style`, `dc-ce-layout-transparent`, `dc-ce-layout-vertical-align`,
  `dc-ce-static-position-size`, `dc-ce-auto-align`, `dc-ce-width`, `dc-ce-height`,
  `dc-ce-margin`, `dc-ce-spacing`, and `dc-ce-frozen`.
- Added the static detector label `Xiumi component depot layout geometry residue` before the broad
  `Xiumi operator depot item residue` fallback, and tightened the broad fallback so the reduced
  layout/geometry component fixture is not double-reported as generic operator-depot residue.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because the reduced fixture emitted only
  `Xiumi operator depot item residue`:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot layout geometry controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "component depot layout geometry controls|component depot native controls|component depot form input controls|operator depot item controls|layout form panels|layout form child" --reporter=default`.
- Verification:
  full platform-rendering regression passed with 1 file and 322 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1299 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  lint, type check, and production build passed:
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules and Vite built in 35.97s.
- Verification:
  release preflight intentionally remains blocked by external gates:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, blocker kinds
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, mobile free-layout interaction, mobile interaction, Dark Mode, cover
  thumbnail acceptance, credentialed sync, free-layout acceptance, native component acceptance,
  scheduled send, platform preview, public article rendering, public-host acceptance,
  XHS/Zhihu account upload, or publish success.
...
## 2026-06-29 Xiumi Panel Header Control Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed panel-header controls
  can remain as `panel-handler`, `panel-placeholder`, `panel-close` paired with `glyphicon`,
  `hammer-handler`, and `comment-panel-header`.
- Added the static detector label `Xiumi panel header control residue` so copied draggable panel
  handles, placeholder icons, close buttons, color-palette drag headers, and comment-panel headers
  are blocked even after paper auxiliary parents, comment-panel parents, color-palette parents,
  operation-bar menus, Angular runtime, hosted media, sidebar, and meta-panel markers are absent.
- Kept the rule anchored to source-specific class/id markers and the `glyphicon` + `panel-close`
  class combination so ordinary panel/header/close/comment/color/placeholder/handler/drag/menu/
  toolbar wording and article text are not blocked by themselves.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "panel header controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "panel header controls|comment toolbar classes|paper auxiliary component tree|operation bar dropdown|color selector controls" --reporter=default`.
- Verification:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --testTimeout=60000`
  passed with 1 file / 299 tests.
- Verification:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --testTimeout=60000`
  passed with 36 files / 1276 tests.
- Verification:
  targeted ESLint, `vue-tsc --noEmit --pretty false`, and production build passed; build
  transformed 4653 modules and completed in 34.99s.
- Release preflight remained correctly blocked by external proof gates:
  `status=blocked-by-external`, `canClaimComplete=false`,
  `blockerKinds=["phone-preview","external-dependency","unsafe-to-automate","mutating-platform"]`,
  `externalHandoffRows=18`, `safeExternalRows=0`, `actionableLocalRows=0`, `nextRowRefs=5`,
...
## 2026-06-29 Xiumi Audio Room Tab Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed the audio/video
  library panel exposes `tn-aud-rooms-tab` and `tn-aud-room-item` on the system music, personal
  music, and personal video tab controls.
- Added the static detector label `Xiumi audio room tab residue` after hidden audio-library
  control handling and before visible audio panel chrome handling.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no `wechat-market-editor-residue`
  issue was emitted:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "audio room tabs" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "audio room tabs|audio library controls|hidden media upload inputs|audio panel controls" --reporter=default`.
- Verification:
  full platform-rendering regression passed with 1 file and 317 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1294 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  lint, type check, and production build passed:
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules in 43.38s.
- Verification:
  release preflight intentionally remains blocked by external gates:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, blocker kinds
...
## 2026-06-29 Xiumi Template List Refresh Directive Addendum

- Added local static publishability protection for Xiumi v5 template/material list refresh
  directives: `tn-pull-to-refresh` now reports the precise
  `Xiumi template list refresh directive residue` label.
- The rule upgrades cleaned-down template/material list refresh fragments from only the broad
  `Xiumi tn-* attribute` fallback to an actionable template-list diagnostic.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because the reduced fixture only reported
  `Xiumi tn-* attribute`; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "template list refresh directives|template entry blocks|template authoring tree|source-house authoring|Xiumi tn\\* attribute" --reporter=default`.
- Full local validation passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --testTimeout=90000`
  passed with 1 file and 342 tests;
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --testTimeout=90000`
  passed with 36 files and 1319 tests;
  targeted ESLint, `vue-tsc --noEmit --pretty false`, and production build passed. The build
  transformed 4653 modules and completed in 41.47s.
- Release preflight remained correctly blocked:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `status=blocked-by-external`, `canClaimComplete=false`,
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC paste,
  phone preview, material-list loading fidelity, template refresh behavior, Dark Mode,
...
- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed the audio/video
  library panel exposes `<audio id="audio-library-control" class="bgm-audio ...">` as a hidden
  editor-side media preview/control surface.
- Added the static detector label `Xiumi audio library control residue` after hidden media upload
  input handling and before audio panel chrome handling.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no `wechat-market-editor-residue`
  issue was emitted:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "audio library controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "audio library controls|hidden media upload inputs|audio panel controls" --reporter=default`.
- Verification:
  full platform-rendering regression passed with 1 file and 316 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1293 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  lint, type check, and production build passed:
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules in 38.25s.
- Verification:
  release preflight intentionally remains blocked by external gates:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, blocker kinds
...
## 2026-06-29 Xiumi Audio Panel Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed the audio/video
  library and background-music controls can expose `audios`, `audio-panel`, `audio-src`,
  `audio-status`, `audio-edit`, `audio-del`, and `audio-group`.
- Added the static detector label `Xiumi audio panel residue` after hidden media upload input
  handling and before account/sync panel rules.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no `wechat-market-editor-residue`
  issue was emitted:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "audio panel controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "audio panel controls|hidden media upload inputs|generated link controls|account sync panel controls" --reporter=default`.
- Verification:
  full platform-rendering regression passed with 1 file and 315 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  full export service regression passed with 36 files and 1292 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  lint, type check, and production build passed:
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  transformed 4653 modules in 39.50s.
- Verification:
  release preflight intentionally remains blocked by external gates:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, blocker kinds
...
- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed editor-side hidden
  color picker triggers can expose `tnColorPickerTrigger` and `tnGradientColorPickerTrigger`.
- Added the static detector label `Xiumi color picker trigger residue` adjacent to color-palette
  panel handling and before color-selector controls.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no `wechat-market-editor-residue`
  issue was emitted:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "color picker triggers" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "color picker triggers|color palette panels|color selector controls" --reporter=default`.
- Full validation:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`
  passed with 1 file and 313 tests;
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`
  passed with 36 files and 1290 tests;
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`
  passed;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`
  passed;
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build`
  passed with 4653 modules transformed in 40.26s.
- Release preflight remained blocked as expected:
  `pnpm -C inkforge style-proof:release-preflight --json`
  exited 1 with `canClaimComplete=false`, `status=blocked-by-external`,
  `blockerKinds=["phone-preview","external-dependency","unsafe-to-automate","mutating-platform"]`,
...
## 2026-06-29 Xiumi Media Upload Input Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed editor-side hidden
  media upload inputs can expose `audioFileUploadInput`, `videoFileUploadInput`,
  `tn-audio-uploader`, and `tn-video-uploader`.
- Added the static detector label `Xiumi media upload input residue` after generated-link controls
  and before account/sync panel rules.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no `wechat-market-editor-residue`
  issue was emitted:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "hidden media upload inputs" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "hidden media upload inputs|generated link controls|account sync panel controls" --reporter=default`.
- Full validation:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`
  ...
  modules and a 34.23s Vite build.
- Release preflight:
...
- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed the editor-side top
  header wrapper can expose `tn-header`.
- Added the static detector label `Xiumi header shell residue` before the broad `Xiumi tn-* attribute`
  fallback so copied top-level header chrome is diagnosed precisely even after `x3-navbar`,
  `x3-nav-*`, operation buttons, prompt banners, Angular runtime classes, UI Bootstrap directives,
  hosted media, sidebar, and meta-panel markers are absent.
- Kept the rule anchored to the source-specific `tn-header` class/id marker so ordinary
  header/navigation/transition/brand/path/login/menu wording, generic `header` elements, and
  `navbar-static-top` classes are not blocked by themselves.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because the fixture reported only the broad
  `Xiumi tn-* attribute` fallback instead of the exact header-shell label:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "header shell without navigation children" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "header shell without navigation children|navigation shell controls|editing dock classes|Angular runtime controls" --reporter=default`.
- Verification:
  platform regression suite passed with 1 file and 309 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  export service suite passed with 36 files and 1286 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  ...
  passed; production build passed after transforming 4653 modules in 38.45s.
- Verification:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 as expected with
...
- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed browser-compatibility
  and copy/plugin operation prompts can expose `tn-compatible-prompt` and `tn-operate-prompt`.
- Added the static detector label `Xiumi editor prompt banner residue` so copied editor prompt
  chrome is blocked even after `tn-working-pallet`, `tn-studio-paper`, `tn-editing-desk`, Angular
  runtime classes, top operation buttons, user profile menus, operation-bar dropdowns, hosted
  media, sidebar, and meta-panel markers are absent.
- Kept the rule anchored to source-specific prompt banner class/id markers so ordinary
  browser/compatibility/copy/plugin/operation/prompt/close/sync/paste wording and article text are
...
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "editor prompt banners" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "editor prompt banners|user profile menu|top operation buttons|Angular runtime controls" --reporter=default`.
- Verification:
  platform regression suite passed with 1 file and 307 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  export service suite passed with 36 files and 1284 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  ESLint passed for `src/services/export/quality-detector.ts` and
  `src/services/export/platform-export-rendering.test.ts`; `vue-tsc --noEmit --pretty false`
  passed; production build passed after transforming 4653 modules in 32.98s.
- Verification:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 as expected with
  `canClaimComplete=false`, `status=blocked-by-external`, blockerKinds `phone-preview`,
  `external-dependency`, `unsafe-to-automate`, and `mutating-platform`; summary included
...
## 2026-06-29 Xiumi User Profile Menu Addendum

- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed the user profile
  dropdown can expose `usr-info-desc-frame` around account settings, invoice, and sign-out chrome.
- Added the static detector label `Xiumi user profile menu residue` so copied account-profile menu
  chrome is blocked even after `wx-user-panel`, `usr-message-box`, `message-box-toggle`, top
  operation buttons, dropdown directives, operation-bar dropdowns, Angular runtime, hosted media,
  sidebar, and meta-panel markers are absent.
- Kept the rule anchored to the source-specific `usr-info-desc-frame` class/id marker so ordinary
  account/profile/settings/invoice/sign-out/user/dropdown/panel wording and article text are not
  blocked by themselves. Generic child markers such as `sign-out`, `nickname`, and `glyphicon`
  remain observed context only.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "user profile menu" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "user profile menu|message panel controls|account sync panel controls|top operation buttons" --reporter=default`.
- Verification:
  platform regression suite passed with 1 file and 306 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`.
- Verification:
  export service suite passed with 36 files and 1283 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`.
- Verification:
  ESLint passed for `src/services/export/quality-detector.ts` and
  `src/services/export/platform-export-rendering.test.ts`; `vue-tsc --noEmit --pretty false`
  passed; production build passed after transforming 4653 modules in 36.35s.
- Verification:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 as expected with
  `canClaimComplete=false`, `status=blocked-by-external`, blockerKinds `phone-preview`,
  `external-dependency`, `unsafe-to-automate`, and `mutating-platform`; summary included
...
- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed the page statistics
  dropdown can remain as `statistics-tool-panel`.
- Added the static detector label `Xiumi statistics panel residue` so copied text-count,
  reading-time, link-count, image-count, and library-usage chrome is blocked even after
  `content-statistics`, right-toolbar, page-toolbar, account/sync, dropdown directive,
  operation-bar, Angular runtime, hosted media, sidebar, and meta-panel markers are absent.
- Kept the rule anchored to the source-specific `statistics-tool-panel` class/id marker so
  ordinary statistics/count/reading-time/link/image/library/panel/dropdown wording and article text
...
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "statistics panel controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "statistics panel controls|right toolbar controls|page toolbar classes|account sync panel controls" --reporter=default`.
- Verification:
  platform regression suite passed with 1 file and 304 tests:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --testTimeout=60000`.
- Verification:
  export service suite passed with 36 files and 1281 tests:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --testTimeout=60000`.
- Verification:
  ESLint passed for `src/services/export/quality-detector.ts` and
  `src/services/export/platform-export-rendering.test.ts`; `vue-tsc --noEmit --pretty false`
  passed; production build passed after transforming 4653 modules in 41.25s.
- Verification:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 as expected with
  `canClaimComplete=false`, `status=blocked-by-external`, blockerKinds `phone-preview`,
  `external-dependency`, `unsafe-to-automate`, and `mutating-platform`; summary included
...
- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed the operation-loader
  area can expose empty editor-side menu pin nodes as `op-cp-menu-pin` and `op-cp-menu-pin-tb`.
- Added the static detector label `Xiumi menu pin control residue` so copied floating/pinned
  operation-menu chrome is blocked even after operation-loader parents, operator dock controls,
  operator depot items, attribute context-menu hosts, attribute-board controls, operation-bar
  menus, Angular runtime, hosted media, sidebar, and meta-panel markers are absent.
- Kept the rule anchored to source-specific `op-cp-menu-pin*` class/id markers so ordinary
  menu/pin/fixed/floating/toolbar/operation wording and article text are not blocked by
  themselves.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "menu pin controls" --reporter=default`.
- Verification:
  focused green and adjacent regressions passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "menu pin controls|operation panel loader classes|operator dock child controls|attribute context menu host controls" --reporter=default`.
- Full platform rendering suite passed:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --testTimeout=60000`
...
- Added local static publishability protection for Xiumi v5 template card hover / feature-match
  markers:
  `inner-image-box`, `lighting-hover`, `comp-feature-matched`, and `large-tpl` now report the
  precise `Xiumi template card hover residue` label.
- The rule covers cleaned-down template card class tokens that previously produced no
  market-editor hard-block for WeChat and no market-editor hard-block for XHS/Zhihu.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "template card hover|visible template card|template renderer pipeline|template scene markers|source-house authoring" --reporter=default`.
- Full local validation passed after the detector update:
...
## 2026-06-29 Xiumi Cover Placeholder Addendum

- Added local static publishability protection for Xiumi v5 cover placeholder / mask shell
  markers:
  `cover-imgs`, `cover-placeholder`, `cover-mask`, `mask-border`, `play-placeholder`, and
  `second-placeholder` now report the precise `Xiumi cover placeholder residue` label.
- The rule covers cleaned-down cover placeholder class tokens that previously produced no
  market-editor hard-block for WeChat and no market-editor hard-block for XHS/Zhihu.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "cover placeholder|WeChat cover controls|WeChat cover menu|dark mask controls|operation bar dropdown" --reporter=default`.
- Full local validation passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=60000`
  passed with 1 file and 295 tests;
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=60000`
  passed with 36 files and 1272 tests;
  targeted ESLint, `vue-tsc --noEmit --pretty false`, and production build passed.
- Release preflight remained correctly blocked:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `status=blocked-by-external`, `canClaimComplete=false`,
...
## 2026-06-29 Xiumi Cover Image Description Addendum

- Extended local static publishability protection for Xiumi v5 cover child markers:
  `cover-img` and `cover-desc` now report the existing precise
  `Xiumi cover placeholder residue` label.
- The rule covers cleaned-down cover image/description child classes that previously produced no
  market-editor hard-block for WeChat and no market-editor hard-block for XHS/Zhihu when the
  broader placeholder shell was absent.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
...
- Full local validation passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --testTimeout=90000`
  passed with 1 file and 337 tests;
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --testTimeout=90000`
  passed with 36 files and 1314 tests;
  targeted ESLint, `vue-tsc --noEmit --pretty false`, and production build passed. The build
  transformed 4653 modules and completed in 36.15s.
- Release preflight remained correctly blocked:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `status=blocked-by-external`, `canClaimComplete=false`,
  `blockerKinds=phone-preview/external-dependency/unsafe-to-automate/mutating-platform`, and
...
- Extended local static publishability protection for the Xiumi v5 background operation child bar:
  `op-cp-bg-bar` now reports under the existing `Xiumi background attribute control residue` label.
- The rule covers a cleaned-down single class token that previously produced no market-editor
  hard-block.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "background bar child controls|background attribute controls|animation attribute panel controls|operator dock child controls|operator depot item controls|attribute board controls" --reporter=default`.
- Full local validation passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=60000`
  passed with 1 file and 293 tests;
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=60000`
  passed with 36 files and 1270 tests;
  targeted ESLint, `vue-tsc --noEmit --pretty false`, and production build passed.
- Release preflight remained correctly blocked:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `status=blocked-by-external`, `canClaimComplete=false`,
...
## 2026-06-29 Xiumi Image Enhancement Crop Control Addendum

- Added local static publishability protection for Xiumi v5 image enhancement, image popup, and
  thumbnail crop child controls:
  `op-cp-image-enhancement`, `op-ce-image-enhancement`, `op-ce-image-popup`,
  `enhance-attr-menu`, and `thumb-crop-img` now report the precise
  `Xiumi image enhancement crop control residue` label.
- The rule covers cleaned-down single class tokens that previously produced no market-editor
  hard-block.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "image enhancement and crop child controls|worker surface crop controls|crop panel child controls|WeChat cover controls|meta panel controls|operator dock child controls" --reporter=default`.
- Full local validation passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=60000`
...
- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed the animation style
  picker can leave isolated `anim-expand-bottom` icon/expand children inside `anim-style` items.
- Extended the static detector label `Xiumi animation style picker residue` so copied
  `anim-expand-bottom` residue is blocked even after broader `anim-style`, `anim-styles`,
  `anim-desc`, `anim-icon`, `animate-styles-type`, and `animate-general` parents have been cleaned.
- Kept the rule anchored to class/id markers so ordinary animation wording, CSS animation
  properties, SVG `<animate>` elements, and motion-related article text are not blocked by
  themselves.
- Added three-platform regression coverage in
...
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "style picker icon" --reporter=default`.
- Verification:
  focused animation-style-picker group passed:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "animation style picker" --reporter=default`
  with 2 selected tests and 284 skipped tests.
- Verification:
  adjacent animation-child and animate-operation regressions passed:
...
- Verification:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --testTimeout=60000`
  passed with 1 file / 286 tests.
- Verification:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --testTimeout=60000`
  passed with 36 files / 1263 tests.
- Verification:
  targeted ESLint, `vue-tsc --noEmit --pretty false`, and production build passed; build
  transformed 4653 modules and completed in 28.93s.
- Release preflight remained correctly blocked by external proof gates:
  `status=blocked-by-external`, `canClaimComplete=false`, `externalHandoffRows=18`,
  `safeExternalRows=0`, `actionableLocalRows=0`, `nextRowRefs=5`, `uniqueNextRows=3`.
- Added evidence file:
...
- Added local static publishability protection for Xiumi v5 operation panel/menu directive
  attributes: `tn-panel-move`, `tn-op-back-mask`, and `tn-op-menu` now report the precise
  `Xiumi operation panel directive residue` label.
- The rule upgrades cleaned-down operation-panel directive fragments from only the broad
  `Xiumi tn-* attribute` fallback to an actionable operation-panel diagnostic.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because the reduced fixture only reported
  `Xiumi tn-* attribute`; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "operation panel directives|operation panel loader|operator dock child|operation panel component controls|Xiumi tn\\* attribute" --reporter=default`.
- Full local validation passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --testTimeout=90000`
  ...
  targeted ESLint, `vue-tsc --noEmit --pretty false`, and production build passed. The build
  transformed 4653 modules and completed in 40.49s.
- Release preflight remained correctly blocked:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `status=blocked-by-external`, `canClaimComplete=false`,
  `blockerKinds=phone-preview/external-dependency/unsafe-to-automate/mutating-platform`, and
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC paste,
  phone preview, H5 panel/menu interaction fidelity, Dark Mode, credentialed sync, scheduled send,
  platform preview, public article rendering, public-host acceptance, XHS/Zhihu account upload, or
  publish success.
...
- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed animation style
  picker controls can remain as `anim-desc`, `anim-icon`, `anim-style`, `anim-styles`,
  `animate-styles-type`, and `animate-general`.
- Added the static detector label `Xiumi animation style picker residue` so copied animation
  effect group, icon, and description controls are blocked even after animation-list,
  animation-attribute, animation-operation, Angular runtime, hosted media, sidebar, and meta-panel
  markers are absent.
- Kept the rule anchored to source-specific class/id markers so ordinary animation wording, CSS
  animation properties, SVG `<animate>` elements, and motion-related article text are not blocked
  by themselves.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  ...
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "animation style picker" --reporter=default`.
- Verification:
  adjacent animation-child, animation-attribute, and animate-operation regressions passed:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "animation panel child" --reporter=default`,
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "animation attribute panel" --reporter=default`,
  and
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "animate operation" --reporter=default`.
- Verification:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --testTimeout=60000`
  passed with 1 file / 285 tests.
- Verification:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --testTimeout=60000`
  passed with 36 files / 1262 tests.
- Verification:
  targeted ESLint, `vue-tsc --noEmit --pretty false`, and production build passed; build
  transformed 4653 modules and completed in 25.50s.
- Release preflight remained correctly blocked by external proof gates:
  `status=blocked-by-external`, `canClaimComplete=false`, `externalHandoffRows=18`,
  `safeExternalRows=0`, `actionableLocalRows=0`, `nextRowRefs=5`, `uniqueNextRows=3`.
- Added evidence file:
...
- Reviewed the live Xiumi v5 paper editor through CloakBrowser and confirmed operation overlay
  masks can remain as `op-dark-mask`.
- Added the static detector label `Xiumi dark mask control residue` so copied editor dark-mask
  surfaces are blocked even after WeChat-cover, generated-link, selection-overlay,
  worker-surface, operator/depot, Angular runtime, hosted media, sidebar, and meta-panel markers
  are absent.
- Kept the rule anchored to source-specific `op-dark-mask` class/id markers so ordinary dark/mask
  wording, image overlay wording, background CSS, and article text are not blocked by themselves.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- Verification:
  focused TDD red failed before the detector update because no market-editor-residue issue was
  emitted; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "dark mask" --reporter=default`.
- Verification:
  adjacent WeChat-cover, selection-overlay, and worker-surface regressions passed:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "WeChat cover controls" --reporter=default`,
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "selection overlay child" --reporter=default`,
  and
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "worker surface crop" --reporter=default`.
- Verification:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --testTimeout=60000`
  passed with 1 file / 283 tests.
- Verification:
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --testTimeout=60000`
  passed with 36 files / 1260 tests.
- Verification:
  targeted ESLint, `vue-tsc --noEmit --pretty false`, and production build passed; build
  transformed 4653 modules and completed in 30.17s.
- Release preflight remained correctly blocked by external proof gates:
  `status=blocked-by-external`, `canClaimComplete=false`, `externalHandoffRows=18`,
  `safeExternalRows=0`, `actionableLocalRows=0`, `nextRowRefs=5`, `uniqueNextRows=3`.
- Added evidence file:
...
## 2026-06-29 Xiumi Image Crop Directive Addendum

- Added local static publishability protection for the live Xiumi v5 image crop directive:
  `tn-img-crop` now reports the precise `Xiumi image crop directive residue` label.
- The rule covers cleaned-down directive-only fragments that previously produced only the broad
  `Xiumi tn-* attribute` diagnostic.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
...
- Verification:
  focused TDD red failed before the detector update because the reduced fixture only emitted
  `Xiumi tn-* attribute`; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "image crop directives|worker surface crop|crop panel child|image enhancement and crop" --reporter=default`.
- Full local validation passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`
...
## 2026-06-29 Xiumi Hammer Pan Directive Addendum

- Extended local static publishability protection for the live Xiumi v5 Hammer gesture attribute:
  `hm-pan` now reports the existing precise `Xiumi selection overlay control residue` label.
- CloakBrowser readback confirmed `hm-pan` appears on the active editor surface alongside
  already-covered `hm-panstart`, `hm-panend`, `hm-panmove`, `hm-recognizer-options`,
  `stop-propagation`, and `tn-attach-to`.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- The reduced fixture keeps only `hm-pan`, so the diagnostic is proven independently from
  selection overlay classes, broader Hammer attributes, broad `tn-*`, Angular runtime attributes,
  hosted media, sidebar controls, or meta panels.
- GitNexus MCP impact reported LOW risk for `MARKET_EDITOR_RESIDUE_RULES` with
  0 direct dependents and LOW risk for `detectQuality` with 4 direct dependents and
  0 affected processes.
- Verification:
  focused TDD red failed before the detector update because no `*-market-editor-residue` issue was
  emitted; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "hammer pan directive|selection overlay child controls|worker surface crop controls" --reporter=default`.
- Full local validation passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`
  ...
  targeted ESLint, `vue-tsc --noEmit --pretty false`, and production build passed.
- Production build transformed 4653 modules and completed in 28.02s.
- Release preflight remained correctly blocked:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `status=blocked-by-external`, `canClaimComplete=false`, `externalHandoffRows=18`,
  `safeExternalRows=0`, `actionableLocalRows=0`, `nextRowRefs=5`, and `uniqueNextRows=3`.
...
- Boundary: this is local static publishability protection only. It does not prove WeChat PC paste,
  phone preview, mobile gesture fidelity, mobile SMIL/click interaction, Dark Mode, credentialed
  sync, scheduled send, platform preview, public article rendering, public-host acceptance,
  XHS/Zhihu account upload, or publish success.

---

## 2026-06-29 Xiumi Style Binding Metadata Addendum

- Added local static publishability protection for the live Xiumi v5 style-binding attribute:
  `tn-style` now reports the precise `Xiumi style binding metadata residue` label.
- CloakBrowser readback confirmed `tn-style` appears on the active editor surface alongside
  already-covered component/cell/page binding metadata.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- The reduced fixture keeps only `tn-style`, so the diagnostic is proven independently from
  ordinary inline `style`, component/cell/page binding metadata, image/link/preload binding
  metadata, Angular runtime attributes, hosted media, sidebar controls, or meta panels.
- GitNexus MCP impact reported LOW risk for `MARKET_EDITOR_RESIDUE_RULES` with
  0 direct dependents and LOW risk for `detectQuality` with 4 direct dependents and
  0 affected processes.
- Verification:
  focused TDD red failed before the detector update because the reduced fixture only emitted
  `Xiumi tn-* attribute`; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "style binding metadata|preload image directives|component structure binding metadata" --reporter=default`.
- Full local validation passed after the detector update:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`
  ...
  targeted ESLint, `vue-tsc --noEmit --pretty false`, and production build passed.
- Production build transformed 4653 modules and completed in 28.34s.
- Release preflight remained correctly blocked:
  `pnpm -C inkforge style-proof:release-preflight --json` exited 1 with
  `status=blocked-by-external`, `canClaimComplete=false`, `externalHandoffRows=18`,
  `safeExternalRows=0`, `actionableLocalRows=0`, `nextRowRefs=5`, and `uniqueNextRows=3`.
...
## 2026-06-29 Xiumi UIB Tab Content Transclude Addendum

- Extended local static publishability protection for the live Xiumi v5 UI Bootstrap tab-body
  directive: `uib-tab-content-transclude` now reports under the existing
  `Xiumi UI Bootstrap control directive residue` label.
- CloakBrowser readback confirmed `uib-tab-content-transclude` appears on the active editor
  surface alongside already-covered UI Bootstrap tab heading, dropdown, tooltip, and button-state
  directives.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`.
- The reduced fixture keeps only `uib-tab-content-transclude`, so the diagnostic is proven
  independently from dropdown wrappers, heading transclusion, Angular runtime classes, `tn-*`,
...
- Verification:
  focused TDD red failed before the detector update because no `*-market-editor-residue` issue was
  emitted; focused green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "tab content directives|UI Bootstrap directives|button directives|dropdown directives" --reporter=default`.
- Full local validation:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --testTimeout=90000`
  passed with 1 file and 356 tests;
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --testTimeout=90000`
  passed with 36 files and 1333 tests;
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`,
  ...
  4653 modules and completed in 36.56s.
- Release preflight remained correctly blocked:
...
## 2026-06-29 135 UEditor Toolbar and Editor Chrome Residue Addendum

- Extended local static publishability protection for live 135 ordinary editor UEditor toolbar and
  editor shell chrome observed with CloakBrowser.
...
- Added `135 UEditor toolbar/editor chrome residue` to `MARKET_EDITOR_RESIDUE_RULES` for copied
  `edui-*` toolbar/editor shell markers and `id="edui..."` controls.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`. The reduced fixture omits
  `_135editor`, `data-tools`, `style_id/style_name/style_price`, left-library operation chrome,
...
- Verification:
  targeted TDD red failed before the detector update because no `*-market-editor-residue` issue
  was emitted; targeted green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "UEditor toolbar" --reporter=default`.
- Full local validation:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`
...
- Release preflight remained correctly blocked:
  ...
  `uniqueNextRows=3`.
- Added evidence file:
  `prompts/0601/evidence/135-ueditor-chrome-residue-20260629.txt`.
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, mobile interaction fidelity, mobile Dark Mode, cover thumbnail
  acceptance, upload, credentialed sync, scheduled send, platform preview, public article
  rendering, public-host acceptance, XHS/Zhihu account upload, or publish success.
...
## 2026-06-29 135 Style Panel Navigation Chrome Residue Addendum

- Extended local static publishability protection for live 135 ordinary editor left-panel style
  navigation chrome observed with CloakBrowser.
- Observed style-panel markers included `style-operate-area`, `style-color-palette`,
  `style-categories`, `style-sorts`, and `news_modal-ys`.
- Added `135 style panel navigation chrome residue` to `MARKET_EDITOR_RESIDUE_RULES` for copied
  style-library navigation, theme-color, and sort controls.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`. The reduced fixture omits
  `_135editor`, `data-tools`, style-list metadata, style-card operation chrome, UEditor toolbar
  chrome, action rail chrome, hosted image sources, and SVG-builder markers, so the new label is
  proven independently from older 135 blockers.
- GitNexus MCP impact reported LOW risk for `MARKET_EDITOR_RESIDUE_RULES` with 0 direct
  dependents and 0 affected processes.
- Verification:
  targeted TDD red failed before the detector update because no `*-market-editor-residue` issue
  was emitted; targeted green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "style panel navigation chrome" --reporter=default`.
- Full local validation:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`
  passed with 1 file and 362 tests;
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`
  passed with 36 files and 1339 tests;
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`,
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`, and
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build` passed; the build transformed
  4653 modules and completed in 27.99 seconds.
- Release preflight remained correctly blocked:
  `pnpm -C inkforge style-proof:release-preflight --json` returned `canClaimComplete=false`,
  `status=blocked-by-external`, blocker kinds `phone-preview`, `external-dependency`,
  `unsafe-to-automate`, and `mutating-platform`, with `blockerCount=4`,
  ...
  `uniqueNextRows=3`.
- Added evidence file:
  `prompts/0601/evidence/135-style-panel-navigation-chrome-residue-20260629.txt`.
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, mobile interaction fidelity, mobile Dark Mode, cover thumbnail
  acceptance, upload, credentialed sync, scheduled send, platform preview, public article
  rendering, public-host acceptance, XHS/Zhihu account upload, or publish success.
...
## 2026-06-29 135 Announcement Link Chrome Residue Addendum

- Extended local static publishability protection for live 135 ordinary editor announcement-strip
  link chrome observed with CloakBrowser.
- Observed announcement link markers included `class="announcement unread"` and
  `https://www.135editor.com/announcements/view/<id>`.
- Added `135 announcement link chrome residue` to `MARKET_EDITOR_RESIDUE_RULES` for copied site
  notice links.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`. The reduced fixture omits
  `_135editor`, `data-tools`, `style_id/style_name/style_price`, full-page navigation chrome,
  helper iframe chrome, style-panel controls, style-card operation buttons, UEditor toolbar
  chrome, action rail chrome, hosted image sources, and SVG-builder markers, so the new label is
  proven independently from older 135 blockers.
- The detector remains source-specific: it does not block ordinary announcement prose.
- GitNexus MCP impact reported LOW risk for `MARKET_EDITOR_RESIDUE_RULES` with 0 direct
  dependents and 0 affected processes.
- Verification:
  targeted TDD red failed before the detector update because no `*-market-editor-residue` issue
  was emitted; targeted green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "announcement link chrome" --reporter=default`.
- Adjacent 135 regression:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "135 (style panel navigation chrome|full-page navigation chrome|helper iframe chrome|announcement link chrome|UEditor toolbar|editor action rail chrome)" --reporter=default`
  passed with 6 selected tests and 359 skipped tests.
- Full local validation:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`
  passed with 1 file and 365 tests;
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`
  passed with 36 files and 1342 tests;
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`,
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`, and
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build` passed; the build transformed
  4653 modules and completed in 30.10 seconds.
- Release preflight remained correctly blocked:
  `pnpm -C inkforge style-proof:release-preflight --json` returned `canClaimComplete=false`,
  `status=blocked-by-external`, blocker kinds `phone-preview`, `external-dependency`,
  `unsafe-to-automate`, and `mutating-platform`, with `blockerCount=4`,
  ...
  `uniqueNextRows=3`.
- Added evidence file:
  `prompts/0601/evidence/135-announcement-link-chrome-residue-20260629.txt`.
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
  paste, phone preview, link fidelity, mobile interaction fidelity, mobile Dark Mode, cover
  thumbnail acceptance, upload, credentialed sync, scheduled send, platform preview, public
  article rendering, public-host acceptance, XHS/Zhihu account upload, or publish success.

---

## 2026-06-29 Xiumi UI Bootstrap Modal Directive Residue Addendum

- Extended local static publishability protection for live Xiumi v5 UI Bootstrap modal directive
  residue observed with CloakBrowser.
- Observed modal marker: `uib-modal-animation-class`.
- Extended the existing `Xiumi UI Bootstrap control directive residue` detector to cover
  `uib-modal*` attributes.
- Added three-platform regression coverage in
  `inkforge/src/services/export/platform-export-rendering.test.ts`. The reduced fixture omits
  dropdown wrappers, tooltip attributes, tab transclusion, button-state controls, Angular/Vue
  authoring attributes, `tn-*`, operation buttons, hosted media, sidebar controls, and meta
  panels, so the label is proven independently from older Xiumi blockers.
- The detector remains source-specific: it does not block ordinary modal prose, animation prose,
  class prose, or Xiumi by itself.
- GitNexus MCP impact reported LOW risk for `MARKET_EDITOR_RESIDUE_RULES` with 0 direct
  dependents and 0 affected processes.
- Verification:
  targeted TDD red failed before the detector update because no `*-market-editor-residue` issue
  was emitted; targeted green passed after it:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "UI Bootstrap modal directives" --reporter=default`.
- Adjacent UI Bootstrap regression:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts -t "UI Bootstrap (directives|tab content directives|button directives|modal directives)" --reporter=default`
  passed with 4 selected tests and 362 skipped tests.
- Full local validation:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`
  passed with 1 file and 366 tests;
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`
  passed with 36 files and 1343 tests;
  `pnpm -C inkforge exec eslint src/services/export/quality-detector.ts src/services/export/platform-export-rendering.test.ts --quiet`,
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`, and
  `NODE_OPTIONS=--max-old-space-size=4096 pnpm -C inkforge build` passed; the build transformed
  4653 modules and completed in 26.47 seconds.
- Release preflight remained correctly blocked:
  `pnpm -C inkforge style-proof:release-preflight --json` returned `canClaimComplete=false`,
  `status=blocked-by-external`, blocker kinds `phone-preview`, `external-dependency`,
  `unsafe-to-automate`, and `mutating-platform`, with `blockerCount=4`,
  ...
  `uniqueNextRows=3`.
- Added evidence file:
  `prompts/0601/evidence/xiumi-uib-modal-directive-residue-20260629.txt`.
- Boundary: this is local static publishability protection only. It does not prove WeChat PC
...
- Re-ran typecheck and production build:
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false` passed;
...
## 2026-07-03 Style Proof External Handoff CLI Addendum

- Added a read-only operator handoff CLI:
  `pnpm -C inkforge style-proof:external-handoff`.
- The command prints committed markdown by default, accepts explicit `--markdown`, emits raw
  committed packet JSON with `--json`, prints `--help`, rejects conflicting output modes, and
  rejects unknown arguments before reading or claiming proof state.
- The command exits 1 while `canClaimComplete=false`, so it cannot be mistaken for a successful
  release or publish gate.
- Extended the committed markdown formatter so each proof row includes issue ids, freshness issue
  ids, proof counters, and artifact counters. Current handoff logs now show
  `style-proof-manifest-proof-stale` for stale authenticated editor proof and
  `style-proof-manifest-requirement-missing` for missing phone/public-host/credentialed-channel
  proof.
- Current local state remains externally blocked:
  `status=blocked-by-external`, `canClaimComplete=false`, `canContinueLocally=false`,
  `externalHandoffRows=19`, `phoneRows=4`, `externalAccountRows=14`, `publicHostRows=1`,
  `unsafeToAutomateRows=10`, `mutatingRows=14`, `safeExternalRows=0`, and
  `actionableLocalRows=0`.
- Verification:
  ...
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false` passed;
  `$env:NODE_OPTIONS='--max-old-space-size=4096'; pnpm -C inkforge build` passed with 4653
  modules transformed and Vite built in 31.09s, then `inkforge/tsconfig.tsbuildinfo` was restored;
  `pnpm -C inkforge style-proof:release-preflight --json` still exits 1 with
  `canClaimComplete=false`.
- Added evidence file:
  `prompts/0601/evidence/style-proof-external-handoff-cli-20260703.txt`.
...
## 2026-07-03 Style Proof Manifest Merge CLI Addendum

- Added a local manifest merge CLI:
  `pnpm -C inkforge style-proof:manifest-merge --file <redacted-manifest.json> [--file <...>]`.
- The command reads each caller-supplied UTF-8 JSON pack through
  `getStyleProofManifestJsonIntakeReport()`, merges accepted manifest rows in memory, and checks
  the merged `{ manifests: [...] }` pack through `getStyleProofManifestIntakeReport()`.
- Text and JSON outputs are sanitized summaries. They include source counts, per-source ordinal
  summaries, merged counts, issue-id counts, blockers, `canWritePack`, `canClaimComplete`, and
  whether optional `--out` wrote a file. They do not print input paths, output paths, raw
  manifest bodies, raw artifact references, browser profile paths, cookies, tokens, HAR
  references, QR payloads, draft URLs, or publish URLs.
- Optional `--out <path>` writes a merged redacted pack only when every source and merged schema
  check has zero errors and zero warnings, merged semantic issue count is zero, at least one
  manifest is accepted, and the target path does not already exist unless `--force` is used.
- Exit-code contract:
  `0` when the merged pack is schema-clean and semantic-clean and any requested `--out` write
  succeeded;
  `1` when JSON parsed but merge/write is blocked by schema, warning, semantic, hygiene,
  empty-pack, or output-exists issues;
  `2` for CLI usage, unreadable files, or write failure.
- Verification:
  ...
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false` passed;
  `$env:NODE_OPTIONS='--max-old-space-size=4096'; pnpm -C inkforge build` passed with 4653
  modules transformed and Vite built in 31.51s, then `inkforge/tsconfig.tsbuildinfo` was restored;
  `pnpm -C inkforge style-proof:manifest-merge --help` exited 0 and printed the read-only merge
  boundary plus exit-code contract;
  `pnpm -C inkforge style-proof:release-preflight --json` still exits 1 with
  `canClaimComplete=false`.
- Added evidence file:
  `prompts/0601/evidence/style-proof-manifest-merge-cli-20260703.txt`.
...
## 2026-07-03 Style Proof JSON Intake BOM Compatibility Addendum

- Fixed Windows-authored JSON compatibility at the service boundary:
  `getStyleProofManifestJsonIntakeReport()` now removes leading UTF-8 BOM characters before
  size, empty, and JSON parse checks.
- The change is deliberately narrow. It does not alter manifest schema validation, semantic proof
  validation, sensitive-artifact detection, freshness windows, cannot-claim logic, release-gate
  logic, or any platform proof gate.
- Added export-service regression coverage proving BOM-prefixed valid JSON is accepted.
- Added `style-proof:manifest-intake` CLI regression coverage proving a BOM-prefixed temp JSON
  file is accepted and still exits according to cannot-claim state rather than parse failure.
- Verification:
  `pnpm -C inkforge exec vitest run src/services/export/platform-export-rendering.test.ts --reporter=default --test-timeout=90000`
  ...
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false` passed;
  `$env:NODE_OPTIONS='--max-old-space-size=4096'; pnpm -C inkforge build` passed with 4653
  modules transformed and Vite built in 32.26s, then `inkforge/tsconfig.tsbuildinfo` was restored;
  `pnpm -C inkforge style-proof:release-preflight --json` still exits 1 with
  `canClaimComplete=false`.
- Added evidence file:
  `prompts/0601/evidence/style-proof-json-intake-bom-compat-20260703.txt`.
- Boundary: this is JSON intake compatibility only. It does not prove WeChat authenticated editor
...
## 2026-07-03 Style Proof External Handoff Filter Addendum
...
- Added CLI filters:
  ...
  and `--next-only`.
- Default markdown output remains compatible, and unfiltered `--json` still emits the raw
  committed handoff packet.
- Filtered JSON now includes the applied `filters`, the original `committedSummary`, and a
  `filteredSummary` so operators can focus on one platform, one proof kind, or only the next
  priority rows without changing release-gate accounting.
- The command remains read-only and keeps exit code 1 while `canClaimComplete=false`; filtering a
  subset of rows is not proof completion.
- Verification:
  `pnpm -C inkforge exec vitest run scripts/style-proof-external-handoff.test.ts --reporter=default --test-timeout=90000`
  passed with 1 file and 8 tests;
  ...
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false` passed;
  `$env:NODE_OPTIONS='--max-old-space-size=4096'; pnpm -C inkforge build` passed with 4653
  modules transformed and Vite built in 33.31s, then `inkforge/tsconfig.tsbuildinfo` was restored;
  `pnpm -C inkforge style-proof:release-preflight --json` still exits 1 with
  `canClaimComplete=false`.
- Added evidence file:
  `prompts/0601/evidence/style-proof-external-handoff-filters-20260703.txt`.
- Boundary: this is local operator handoff filtering only. It does not prove WeChat authenticated
  editor access, ordinary rich paste retention, phone preview, mobile SMIL/click, mobile Dark
  Mode, cover thumbnail acceptance, upload, credentialed sync, scheduled send, public article
  rendering, Zhihu public-host acceptance, XHS/Zhihu account upload, or publish success.
...
## 2026-07-03 Style Proof External Handoff Template Addendum

- Extended the read-only external handoff command:
  `pnpm -C inkforge style-proof:external-handoff`.
- Added `--template`, a one-line JSON operator worksheet mode for the visible handoff rows.
- `--template` is mutually exclusive with `--markdown` and `--json`, and composes with existing
  `--platform`, `--kind`, `--status`, `--issue`, `--freshness-only`, and `--next-only` filters.
- Template output is explicitly non-proof: `templateOnly:true`, `notProof:true`,
  `canClaimComplete:false`, blank artifact fields, row ids for next-row references, and no
  accepted proof artifact creation.
- The command remains read-only. It does not open a browser, write manifests, upload, sync,
  schedule, publish, or change release-gate accounting.
- Verification:
  `pnpm -C inkforge exec vitest run scripts/style-proof-external-handoff.test.ts --reporter=default --test-timeout=90000`
  ...
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false` passed;
  `$env:NODE_OPTIONS='--max-old-space-size=4096'; pnpm -C inkforge build` passed with 4653
  modules transformed and Vite built in 30.59s, then `inkforge/tsconfig.tsbuildinfo` was restored;
  ...
  exited 1 as expected with one stale authenticated-editor worksheet row;
  `pnpm --silent -C inkforge style-proof:release-preflight --json` still exits 1 with
  `canClaimComplete=false`.
- Added evidence file:
  `prompts/0601/evidence/style-proof-external-handoff-template-20260703.txt`.
- Boundary: this is operator worksheet generation only. It does not prove WeChat authenticated
  editor access, ordinary rich paste retention, phone preview, mobile SMIL/click, mobile Dark
  Mode, cover thumbnail acceptance, upload, credentialed sync, scheduled send, public article
  rendering, Zhihu public-host acceptance, XHS/Zhihu account upload, or publish success.
...
- Verification:
  `pnpm -C inkforge exec vitest run scripts/style-proof-external-handoff.test.ts --reporter=default --test-timeout=90000`
  passed with 1 file and 11 tests, including a cross-CLI manifest-intake regression for the empty
  draft skeletons;
  `pnpm -C inkforge exec eslint scripts/style-proof-external-handoff.ts scripts/style-proof-external-handoff.test.ts --quiet`
  passed;
  `pnpm -C inkforge exec vitest run scripts --reporter=default --test-timeout=90000 --maxWorkers=1 --no-file-parallelism`
...
## 2026-07-03 Style Proof Manifest Intake Next Proof Steps Addendum

- Added sanitized `nextProofSteps` to `pnpm -C inkforge style-proof:manifest-intake --json`.
- Added a `next proof steps:` section to the text report.
- The new rows are derived from the existing execution runbook cannot-claim rows for the supplied
  manifest pack. They expose the required channel/action/readback/field contract, forbidden
  fields, accepted host statuses, freshness limit, redaction boundary, success criteria, failure
  signals, cannot-claim reason, and next operator action.
- This does not import, create, merge, or mark any artifact complete. It only makes the local
  intake report harder to misuse after an operator fills a manifest pack.
- A temporary local empty WeChat/Zhihu draft pack stayed non-claimable:
  `status=ready-for-review`, `canClaimComplete=false`, `artifactCount=0`,
  `nextProofSteps=34`.
...
- Added sanitized evidence:
  `prompts/0601/evidence/style-proof-manifest-intake-next-proof-steps-20260703.txt`.
- Verification passed:
  `pnpm -C inkforge exec vitest run scripts/style-proof-manifest-intake.test.ts --reporter=default --test-timeout=90000`
  with 1 file and 10 tests;
  focused ESLint for `scripts/style-proof-manifest-intake.ts` and its test file;
  `pnpm -C inkforge exec vitest run scripts --reporter=default --test-timeout=90000 --maxWorkers=1 --no-file-parallelism`
  with 4 files and 36 tests;
  `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`
  with 36 files and 1350 tests;
  `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`;
  production build with Vite built in 34.48s.
- Release preflight remains blocked as required:
  `pnpm --silent -C inkforge style-proof:release-preflight --json` exits 1 with
  `status=blocked-by-external`, `canClaimComplete=false`, `blockerCount=4`,
  `combinedIssueCount=11`, `externalHandoffRows=15`, `safeExternalRows=0`, and
  `actionableLocalRows=0`.
- Boundary: this is manifest-intake runbook reporting only. It does not prove WeChat phone
...
## 2026-07-03 Xiumi Applied Center Delta Addendum
...
- The center `.tn-editing-panel` baseline already contained applied SVG-gallery/game-style
  ...
  `29`.
- Left taxonomy exposed title groups, card groups, and SVG groups including image carousel,
  click expand, path animation, lottery, slide, transitions, parallax, click switch, flip, zoom,
  quiz, bullet text, click show/change/open/disappear/popup/zoom/print/jump/play, long-press
  switch, region trigger, falling, and click-plus-auto.
...
- SVG subcategory inspection exposed active taxonomy including `文字弹幕`; a visible left template
  interaction changed the center to hash `2130047643`, HTML length `719571`, `tnComp=193`,
  `tnCell=103`, `tnUuid=93`, `contenteditable=19`, images `98`, and visible
  `马到功成 / 马上有钱` text.
- Cross-check showed no detector source-code change was required: the observed classes and
  attributes are already covered by current Xiumi image-instance, content-overlap/state/editing,
  sortable, operation-panel, template authoring/tree/scene, animation/link binding, runtime path,
  contenteditable, and third-party image residue families.
- Added sanitized evidence:
  `prompts/0601/evidence/xiumi-applied-center-delta-20260703.txt`.
...
## 2026-07-04 Publish Center Rich-Copy Sanitizer Hardening Addendum

- Added `src/services/export/publish-copy.ts` with shared
  `sanitizePublishRichCopyHtml()`, `PUBLISH_COPY_ALLOWED_TAGS`, and
  `PUBLISH_COPY_ALLOWED_ATTR`.
- Exported the sanitizer API through `src/services/export/index.ts`.
- Updated `PublishView.vue` so the rich-copy `execCommand` fallback uses the shared sanitizer.
- Hardened the fallback to fail closed when `document.execCommand('copy')` returns `false`.
- Ensured the temporary fallback DOM node is always removed and the temporary selection is cleared
  in `finally`.
- Sanitizer behavior:
  - keeps the current WeChat-safe SVG subset required by flagship presets:
    `data-ink-svg`, `data-ink-block`, inline `<svg>`, `viewBox`, `width`, `height`, geometry,
    paint, typography, transform, and safe SMIL timing attributes;
  - removes `script`, `style`, `foreignObject`, event-handler attributes, and `javascript:`
    URI values after DOMPurify as a defense-in-depth pass.
- First focused regression run failed before the post-sanitize pass because the previous path
  retained `<style>` and an SVG `onload` attribute in the test DOM. That failure was treated as a
  real hardening gap and fixed.
- Verification passed:
  - `pnpm -C inkforge exec vitest run src/services/export/publish-copy.test.ts src/views/__tests__/PublishView.wechat-presets.test.ts --reporter=default --test-timeout=90000`: 2 files / 8 tests passed.
  - `pnpm -C inkforge exec eslint src/services/export/publish-copy.ts src/services/export/publish-copy.test.ts src/services/export/index.ts src/views/PublishView.vue src/views/__tests__/PublishView.wechat-presets.test.ts --quiet`: passed.
...
- Added sanitized evidence:
  `prompts/0601/evidence/publish-center-rich-copy-sanitizer-20260704.txt`.
- Boundary: this proves local Publish Center rich-copy fallback payload readiness only. It does not
...
## 2026-07-04 WeChat Manual External Proof Handoff Addendum

- Generated the current manual operator handoff for the remaining in-scope WeChat external gates:
  phone-preview proof and credentialed-account proof.
- This follows the narrowed round target: application-level SVG/style availability plus WeChat
  applicability. Xiaohongshu and Zhihu publish-side automation is intentionally deferred for the
  operator's manual testing in this round.
- The handoff records the current local release state:
  - `status=blocked-by-external`;
  - `canClaimComplete=false`;
  - `actionableLocalRows=0`;
  - 20 committed local manifests, 9 committed WeChat PC manifests, and 29 combined committed
    manifests;
  - 8 external handoff rows, split into 4 phone-preview rows and 4 credentialed-account rows;
  - 7 manually deferred non-WeChat rows, split into Xiaohongshu 2 and Zhihu 5.
- Generated and reviewed the current handoff templates/drafts with:
  ...
  - `pnpm --silent -C inkforge style-proof:external-handoff --manifest-drafts --platform=wechat --kind=phone-preview --status=blocked-by-external --issue=style-proof-manifest-requirement-missing --next-only`;
  ...
  - `pnpm --silent -C inkforge style-proof:external-handoff --manifest-drafts --platform=wechat --kind=external-account --status=unsafe-to-automate --issue=style-proof-manifest-requirement-missing --next-only`.
- The external-handoff commands intentionally exit non-zero while `notProof=true` and
  `canClaimComplete=false`; that is the correct behavior until real redacted phone/account proof is
  collected and accepted.
- Added sanitized evidence:
  `prompts/0601/evidence/wechat-manual-external-handoff-20260704.txt`.
- The handoff file records required fields, forbidden blocker fields, and safe intake commands for:
  - phone preview, cover thumbnail, Dark Mode, and phone screenshot readback;
  - credentialed channel response, sync readback, scheduled send readback, and platform preview
    readback.
- Boundary: this is a manual verification handoff only. It does not prove WeChat phone preview,
...
## 2026-07-04 ExportModal WeChat SVG Application Slots Addendum

- Added an app-facing WeChat SVG slot service:
  `src/services/export/wechat-svg-application.ts`.
- Re-exported the slot service and SVG module registry helpers from
  `src/services/export/index.ts`.
- Added an opt-in `SVG 高级排版模块` section to `ExportModal.vue` with five semantic slots:
  - `cover`;
  - `heading`;
  - `divider`;
  - `blockquote`;
  - `showcase`.
- The `showcase` slot exposes all 27 current `SVG_MODULES` entries in registry order, so the app
  can apply every owned SVG module without copying 135/Xiumi source DOM, vendor classes, hosted
  material URLs, or account artifacts.
- Slot selections write through to the existing export option contract:
  - `enableSvgModules`;
  - `svgInjectionPlan`.
- The ExportModal render path remains the existing real WeChat export call:
  `markdownToWechatWithStats(props.content, preset, exportOptions.value)`.
- Default behavior remains unchanged: the SVG slot section is disabled until the operator enables
  it.
- Extended application preflight with:
...
- Verification passed:
  - `pnpm -C inkforge exec vitest run src/services/export/wechat-svg-application.test.ts src/components/export/ExportModal.svg-options.test.ts src/services/export/wechat-svg-options.test.ts scripts/style-proof-release-preflight.test.ts --reporter=default --test-timeout=90000 --maxWorkers=1 --no-file-parallelism`: 4 files / 15 tests passed.
  - `pnpm --silent -C inkforge style-proof:release-preflight --scope=application --json`: exited 0 with `status=application-ready`, `wechatApplicationSvgSlotCount=5`, `wechatApplicationSvgShowcaseModuleCount=27`, `wechatApplicationSvgSlotFailureCount=0`, `wechatApplicationSlotIssues=[]`, `wechatOptionInjectedModuleCount=27`, and `wechatOptionInjectionFailureCount=0`.
  - `pnpm -C inkforge exec eslint src/components/export/ExportModal.vue src/components/export/ExportModal.svg-options.test.ts src/services/export/wechat-svg-application.ts src/services/export/wechat-svg-application.test.ts src/services/export/wechat-svg-options.ts src/services/export/wechat-svg-options.test.ts src/services/export/index.ts scripts/style-proof-release-preflight.ts scripts/style-proof-release-preflight.test.ts --quiet`: passed.
  - `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`: passed.
  ...
  - CloakBrowser visual/DOM check against the local Vite app confirmed the ExportModal section
    `微信公众号 SVG 高级排版模块`, default-off summary, five semantic slots, and the 27-option
...
- Added sanitized evidence:
  `prompts/0601/evidence/exportmodal-wechat-svg-slots-20260704.txt`.
- Boundary: this proves local application-level SVG/style selection and WeChat-safe export
  readiness only. It does not prove WeChat ordinary paste, phone preview, mobile Dark Mode, mobile
  interaction, cover thumbnail acceptance, credentialed sync, scheduled send, public rendering,
  platform preview, or publish success. Xiaohongshu and Zhihu publish-side tests are manually
  deferred to the user for this round and are not claimed by this artifact.
...
## 2026-07-04 Publish Center WeChat SVG Application Slots Addendum

- Added an opt-in `SVG 高级排版模块` section to `PublishView.vue` for the WeChat platform.
- The section reuses the existing app-facing WeChat SVG slot service and exposes five semantic
  slots:
  - `cover`;
  - `heading`;
  - `divider`;
  - `blockquote`;
  - `showcase`.
- The `showcase` slot exposes all 27 current `SVG_MODULES` entries in registry order, including
  interaction-family modules, so the publish center can apply every owned SVG module without
  copying 135/Xiumi source DOM, vendor classes, hosted material URLs, or account artifacts.
- Slot selections write through to the publish-center export option contract:
  - `enableSvgModules`;
  - `svgInjectionPlan`.
- The PublishView render path remains the existing real WeChat export call:
  `markdownToWechatWithStats(content.value, preset, options)`.
- Default behavior remains unchanged: the SVG slot section is disabled until the operator enables
  it.
- Verification passed:
  - `pnpm -C inkforge exec vitest run src/views/__tests__/PublishView.wechat-presets.test.ts src/services/export/wechat-svg-application.test.ts src/services/export/wechat-svg-options.test.ts scripts/style-proof-release-preflight.test.ts --reporter=default --test-timeout=90000 --maxWorkers=1 --no-file-parallelism`: 4 files / 20 tests passed.
  - `pnpm -C inkforge exec eslint src/views/PublishView.vue src/views/__tests__/PublishView.wechat-presets.test.ts src/services/export/wechat-svg-application.ts src/services/export/wechat-svg-application.test.ts --quiet`: passed.
  - `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`: passed.
  - `pnpm --silent -C inkforge style-proof:release-preflight --scope=application --json`: exited 0 with `status=application-ready`, `canClaimApplicationReady=true`, `wechatApplicationSvgSlotCount=5`, `wechatApplicationSvgShowcaseModuleCount=27`, `wechatApplicationSvgSlotFailureCount=0`, `wechatApplicationSlotIssues=[]`, `wechatOptionInjectedModuleCount=27`, and `wechatOptionInjectionFailureCount=0`.
  - `$env:NODE_OPTIONS='--max-old-space-size=4096'; pnpm -C inkforge build`: passed; generated `inkforge/tsconfig.tsbuildinfo` was restored afterward.
  - CloakBrowser visual/DOM check against the local Vite app confirmed the PublishView `SVG 高级排版模块` section, default-off summary, five semantic slots, and the 27-option all-module trial slot.
  - CloakBrowser interaction proof enabled the section, verified all five selectors were available, selected `i-stretch` in the all-module slot, and reported zero browser console errors.
- Added sanitized evidence:
  `prompts/0601/evidence/publishview-wechat-svg-slots-20260704.txt`.
...
## 2026-07-04 Application Preflight Gallery Readiness Addendum

- Strengthened `style-proof:release-preflight --scope=application` so the narrowed local round
  gate now consumes the real application SVG gallery service readiness.
- The application JSON report now includes:
...
- `canClaimApplicationReady` now requires `applicationGalleryStatus=application-gallery-ready`
  and zero gallery issues, in addition to the existing module, application-slot, UI-surface,
  WeChat export-envelope, option-injection, style-choice, local-actionability, and
  local-conflict gates.
- TDD record:
  - The first focused run failed before implementation because the new gallery-readiness fields
    were absent.
  - The post-implementation focused run passed with 1 file / 6 tests.
- Verification:
  - `pnpm -C inkforge exec vitest run scripts/style-proof-release-preflight.test.ts --reporter=default --test-timeout=90000 --maxWorkers=1 --no-file-parallelism`: 1 file / 6 tests passed.
  - `pnpm -C inkforge exec vitest run scripts/style-proof-release-preflight.test.ts src/services/export/application-svg-gallery.test.ts scripts/style-proof-application-gallery.test.ts src/services/export/wechat-svg-application.test.ts src/services/export/wechat-svg-options.test.ts src/components/export/ExportModal.svg-options.test.ts src/views/__tests__/PublishView.wechat-presets.test.ts --reporter=default --test-timeout=90000 --maxWorkers=1 --no-file-parallelism`: 7 files / 27 tests passed.
  - `pnpm --silent -C inkforge style-proof:release-preflight --scope=application --json`: exited 0 with `status=application-ready`, `applicationGalleryStatus=application-gallery-ready`, 108 rendered gallery pairs, 0 WeChat-safe violations, 0 sentinel failures, and 0 gallery issues.
  - `pnpm --silent -C inkforge style-proof:application-gallery --json --out <non-repo-output>`: exited 0 with `status=application-gallery-ready`, 27 modules, 7 families, 4 personas, 108 rendered pairs, and 0 issues.
  ...
  - Focused ESLint for touched preflight, gallery, WeChat SVG application/option, ExportModal, and PublishView files: passed.
  - `pnpm -C inkforge exec vitest run src/services/export --reporter=default --maxWorkers=1 --no-file-parallelism --test-timeout=90000`: 40 files / 1363 tests passed.
  - `pnpm -C inkforge exec vue-tsc --noEmit --pretty false`: passed.
  - `$env:NODE_OPTIONS='--max-old-space-size=4096'; pnpm -C inkforge build`: passed; generated `inkforge/tsconfig.tsbuildinfo` was restored afterward.
- Added sanitized evidence:
  `prompts/0601/evidence/application-preflight-gallery-readiness-20260704.txt`.
- Boundary: this proves local application SVG/style gallery readiness and WeChat-safe local
  output readiness only. It does not prove WeChat ordinary paste, phone preview, mobile Dark

...
- Shared Tauri desktop E2E must select the main WebView, maximize it, and fail closed until a desktop-interactable viewport is present before any route proof starts. A real minimize/focus-loss case must restore the same window handle and foreground focus before later tests; do not accept cascaded `element not interactable` failures as unrelated product defects.

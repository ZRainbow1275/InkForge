...
## 2026-07-03 Style Proof External Handoff Filters

- [x] style-proof-external-handoff-filters-20260703.txt
- Extended `pnpm -C inkforge style-proof:external-handoff` with read-only operator filters:
...
- Filtered JSON includes `filters`, `committedSummary`, and `filteredSummary` to keep operator
  focus separate from committed release-gate accounting.
- The command still exits 1 while `canClaimComplete=false`; a filtered row list is not proof that
  phone/account/public-host/platform rows were completed.
- Verification passed:
  `pnpm -C inkforge exec vitest run scripts/style-proof-external-handoff.test.ts --reporter=default --test-timeout=90000`,
  ...
  `$env:NODE_OPTIONS='--max-old-space-size=4096'; pnpm -C inkforge build`, and
  `pnpm -C inkforge style-proof:release-preflight --json`.
  Script coverage passed with 4 files and 29 tests; export service coverage passed with 36 files
  and 1350 tests; production build transformed 4653 modules and built in 33.31s.
- `pnpm -C inkforge style-proof:release-preflight --json` still exits 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, `externalHandoffRows=19`,
  `safeExternalRows=0`, and `actionableLocalRows=0`.
...
## 2026-07-03 Style Proof External Handoff Status/Freshness Filters

- [x] style-proof-external-handoff-status-filters-20260703.txt
- Extended `pnpm -C inkforge style-proof:external-handoff` with read-only operator filters:
  `--status <completed|missing|invalid|blocked-by-external|unsafe-to-automate>`,
  `--issue <issue-id>`, and `--freshness-only`.
- The filters compose with existing `--platform`, `--kind`, and `--next-only` without changing
  release-gate accounting or the cannot-claim exit-code contract.
- Filtered JSON now includes `statuses`, `issueIds`, and `freshnessIssueRows` inside
  `filteredSummary`; filtered markdown prints the same filter dimensions in its header.
- Verified stale proof isolation with:
  ...
  It exited 1 as expected and returned one stale authenticated-editor row, not proof completion.
- Verification passed:
  `pnpm -C inkforge exec vitest run scripts/style-proof-external-handoff.test.ts --reporter=default --test-timeout=90000`,
  `pnpm -C inkforge exec eslint scripts/style-proof-external-handoff.ts scripts/style-proof-external-handoff.test.ts --quiet`,
  `pnpm -C inkforge exec vitest run scripts --reporter=default --test-timeout=90000 --maxWorkers=1 --no-file-parallelism`,
  ...
  Script coverage passed with 4 files and 31 tests; export service coverage passed with 36 files
  and 1350 tests; production build transformed 4653 modules and built in 32.13s.
...
## 2026-07-03 Style Proof External Handoff Template

- [x] style-proof-external-handoff-template-20260703.txt
- Added `--template` to `pnpm -C inkforge style-proof:external-handoff` as a read-only operator
  worksheet mode.
- The mode composes with `--platform`, `--kind`, `--status`, `--issue`, `--freshness-only`, and
  `--next-only` so a filtered stale or missing handoff row can be converted into blank collection
  guidance.
- Template JSON is explicitly non-proof with `templateOnly:true`, `notProof:true`,
  `canClaimComplete:false`, blank artifact fields, and row ids for next-row references.
- It does not create proof artifacts, write manifest files, open a browser, upload, sync,
  schedule, publish, or change release-gate accounting.
- Verified stale worksheet generation with:
  ...
  It exited 1 as expected and returned one stale authenticated-editor worksheet row, not proof
  completion.
- Verification passed:
  `pnpm -C inkforge exec vitest run scripts/style-proof-external-handoff.test.ts --reporter=default --test-timeout=90000`,
  `pnpm -C inkforge exec eslint scripts/style-proof-external-handoff.ts scripts/style-proof-external-handoff.test.ts --quiet`,
  ...
  `$env:NODE_OPTIONS='--max-old-space-size=4096'; pnpm -C inkforge build`, and
  `pnpm --silent -C inkforge style-proof:release-preflight --json`.
  Focused handoff coverage passed with 1 file and 10 tests; script coverage passed with 4 files
  and 32 tests; export service coverage passed with 36 files and 1350 tests; production build
  transformed 4653 modules and built in 30.59s.
- `pnpm --silent -C inkforge style-proof:release-preflight --json` still exits 1 with
  `canClaimComplete=false`, `status=blocked-by-external`, `externalHandoffRows=19`,
  `safeExternalRows=0`, and `actionableLocalRows=0`.
- Boundary: this is local operator worksheet generation only. It does not prove WeChat
  authenticated editor access, ordinary rich paste retention, phone preview, mobile interaction,
  mobile Dark Mode, cover thumbnail acceptance, credentialed sync, scheduled send, public
  rendering, Zhihu public-host acceptance, XHS/Zhihu account upload, or publish success.
...
- The aggregate now includes `wechat-manual-template` between `wechat-manual-checklist` and
  `wechat-manual-manifest-drafts`, with `status=manual-template-ready` and

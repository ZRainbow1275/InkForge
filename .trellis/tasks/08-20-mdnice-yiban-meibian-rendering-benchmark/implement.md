...
Planning approved on 2026-08-21. Do not run `task.py start` until the implementation baseline gate is satisfied; external WeChat actions still require a separate preflighted batch approval.

## 0. Entry gates

- [ ] Bind an exact branch/commit containing `markdownToWechatWithStats()`, the existing clipboard path, `publishWechatDraft()`, StyleProof/manual manifest and current tests.
- [ ] If a required capability exists only in the dirty tree, first land it in a separate reviewable baseline or revise this plan.
- [ ] Record the bound commit and three usable catalog choices in task metadata/research.
- [ ] Re-run GitNexus staleness check. Before editing any symbol, run upstream impact and warn on HIGH/CRITICAL risk.
- [ ] Explicitly read all relevant portions of `.trellis/spec/frontend/wechat-svg-modules.md`; its JSONL injection is truncated at 32 KiB and cannot substitute for source reading.
- [ ] Obtain user approval of this latest planning summary before local implementation.

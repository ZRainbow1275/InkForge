...
> 证据清单也以 `style-catalog.ts` 为准：`getEvidenceProofRequirements()` / `getStyleChoiceProofRequirements()` 只列出需要补齐的真实证明项，不会把 `blocked` 样式升级为可用。
> 证据采集排程也以 `style-catalog.ts` 为准：`getPlatformStyleProofCollectionPlan()` 把 readiness 缺口拆成本地证据、市场编辑器、微信 PC 编辑器、手机预览、授权通道、public host、发布和敏感清洁门禁；它只排程，不证明任何门禁通过。
...
- 2026-06-19 CloakBrowser applied-rule refresh 确认：135 的空白 `_135editor` 插入只能算插入风险；135 SVG trigger canvas 和秀米 SVG/H5 flow-canvas 作者态只能转成 InkForge 自有 trigger-zone、image-slot、motion/action schema 和 fallback，不得把 `app-content-canvas`、`ant-tooltip-open`、`tn-svg-animation-*`、`tn-yzk-font-*`、`tn-placeholder`、`opera-tn-ra-*` 或 Angular `ng-*` 带入微信输出。
- 2026-06-08 `flagship-amber` 曾有真实微信普通剪贴板负向证据：artifact 剪贴板中含富 HTML/SVG，但 `Control+V` 后编辑器 DOM 读回为纯文本（`data-ink-svg=0`、`svg=0`、`style=0`）。该历史负向证据不能被本地/Tauri/export artifact 外推覆盖。
- 2026-06-09 `flagship-amber` 已有 CloakBrowser 程序化 `ClipboardEvent('paste')` + `DataTransfer` 的真实微信 PC 编辑器读回：exact artifact 保留 `data-ink-svg=3` / `svg=35`。该证据只覆盖这个 PC channel，不能替代手机预览、Dark Mode、封面缩略图、同步、定时发送或发布证明。
- 2026-06-18 `flagship-amber` 后续用 CloakBrowser-only 普通 OS Ctrl+V exact proof 证明 raw `flagship-amber.html` 可进入真实微信 PC 正文编辑器并保留 `svg=35` / `data-ink-svg=3`，且 disposable draft 与 residual draft 已清理。当前 runtime catalog 因此把 Amber 设为 `available`，但仅覆盖 PC 粘贴地板，不覆盖手机预览、SMIL/点击、Dark Mode、封面缩略图、同步、定时发送或发布。
...
- 同一只读复核观察到 `#js_add_appmsg` / `data-action="add"` 的“增加一条/新建内容”入口。该入口会改变真实多图文草稿结构；没有 disposable draft 与 cleanup proof 时不得点击，不能冒充安全粘贴验证。

...
- P1-05 已按本规范完成并重新验证：`FloatingToolbar.vue` 保留无固定顶部工具栏策略，完成左右夹紧、上方空间不足翻转和窄屏 wrap。
- `FloatingToolbar.vue` 当前包含 28 个 title；删除线、行内代码、引用、代码块、分割线、表格等关键快捷键已与当前 Settings-backed 快捷键定义对齐。
- `EditorContextMenu.vue` 已集成到 `EditorPanel.vue`，提供 15 个真实动作并包含视口边界处理、ESC 关闭和外部点击关闭。
- `SlashCommands.ts` 当前为 21 个真实命令，新增 h4、paragraph、link、highlight、textColor、alignCenter、alignRight、callout、details、clearFormat；`SlashCommandMenu.vue` 按 heading / block / list / insert / advanced 分组。
- `/image` 不使用 `window.prompt()`，而是进入真实文件选择器与 asset pipeline；未引入 mock 数据或 Emoji 图标。
- 验证通过：`P1_05_STATIC_OK`、`pnpm exec vue-tsc --noEmit`、`pnpm exec eslint src --ext .ts,.tsx,.vue --quiet`、`pnpm build`。

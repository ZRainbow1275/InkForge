...
**文件**: `components/hub/insights/WritingTimeline.vue`
...
**文件**: `components/hub/insights/ProductivityInsights.vue`
...
**文件**: `components/hub/insights/WordDistribution.vue`
...
**文件**: `components/hub/insights/TagCloud.vue`
...
**文件**: `components/hub/insights/RecentActivity.vue`
...
**文件**: `components/hub/insights/ExportFrequency.vue`
...
| 操作 | 文件路径 | 说明 |
|---|---|---|
| 新增 | `components/hub/insights/WritingTimeline.vue` | 写作活动时间线 (7-14 天) |
| 新增 | `components/hub/insights/ProductivityInsights.vue` | 生产力洞察 (6 项指标, 2x3 网格) |
| 新增 | `components/hub/insights/WordDistribution.vue` | 字数分布直方图 (6 个桶) |
| 新增 | `components/hub/insights/TagCloud.vue` | 标签词云 (flex-wrap 布局) |
| 新增 | `components/hub/insights/RecentActivity.vue` | 最近活动日志 (最多 10 条) |
| 新增 | `components/hub/insights/ExportFrequency.vue` | 导出频率分布 (水平柱状图) |
| 新增 | `components/hub/insights/ContributionHeatmap.vue` | 真实数据热力图 + 空状态 UI |
| 新增 | `components/hub/insights/WordCountTrend.vue` | 真实字数趋势 + 空状态 UI |
| 新增 | `components/hub/insights/CategoryDistribution.vue` | 真实分类分布 + 空状态 UI |
| 新增 | `components/hub/insights/DataInsightsSection.vue` | Section 3 组合容器与响应式布局 |
| 修改 | `views/HubView.vue` | 接入 DataInsightsSection 到 Hub 筛选栏前 |

## 八、验收标准

- [x] 数据洞察区包含 9 个不同的图表/卡片 (3 现有 + 6 新增)
- [x] 所有图表使用真实数据 (来自 ArticleStore / CategoryStore；activity_logs 未落库时使用 articles 时间推断)
- [x] 所有 9 个图表在无数据时显示友好的空状态 (lucide 图标 + 提示文字)
- [x] 桌面/平板/手机三端布局正确
- [x] 数字有 count-up 动画 (ProductivityInsights)
- [x] 颜色方案与品牌一致 (#D32F2F 系列)
- [x] 图表可响应数据变化 (reactive, 使用 computed)
- [x] 所有图标使用 lucide-vue-next (无 Emoji)
- [x] 无 Mock 数据
- [x] 无外部图表库依赖 (纯 SVG + CSS 实现)

## 2026-04-29 Implementation Ledger

- P1-08 已在 `inkforge/src/components/hub/insights/` 下完成 9 张卡片组件与 `DataInsightsSection` 组合容器，`HubView` 已接入 Section 3。
- 本实现使用 Vue computed 从真实 `ArticleStore` / `CategoryStore` 传入数据派生 heatmap、趋势、分类、时间线、生产力、字数分布、标签云与最近活动。
- `ExportFrequency` 保持真实日志边界：当前代码库没有 `export_logs` 持久化表时只显示空状态，不用平台常量制造导出次数。
- 所有图标来自 `lucide-vue-next`，图表使用 SVG/CSS，无外部 chart dependency，无 mock / sample / fixture。
